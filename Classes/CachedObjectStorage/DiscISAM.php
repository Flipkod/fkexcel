...
/**
 * FKExcel_CachedObjectStorage_DiscISAM
 *
 * @category   FKExcel
 * @package    FKExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_CachedObjectStorage_DiscISAM extends FKExcel_CachedObjectStorage_CacheBase implements FKExcel_CachedObjectStorage_ICache {
	...
	public function copyCellCollection(FKExcel_Worksheet $parent) {
		...
		$newFileName = $this->_cacheDirectory.'/FKExcel.'.$baseUnique.'.cache';
	...
	public function __construct(FKExcel_Worksheet $parent, $arguments) {
		$this->_cacheDirectory	= ((isset($arguments['dir'])) && ($arguments['dir'] !== NULL))
									? $arguments['dir']
									: FKExcel_Shared_File::sys_get_temp_dir();

		parent::__construct($parent);
		if (is_null($this->_fileHandle)) {
			$baseUnique = $this->_getUniqueID();
			$this->_fileName = $this->_cacheDirectory.'/FKExcel.'.$baseUnique.'.cache';
			$this->_fileHandle = fopen($this->_fileName,'a+');
		}
