...
/**
 * FKExcel_CachedObjectStorage_Memcache
 *
 * @category   FKExcel
 * @package    FKExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_CachedObjectStorage_Memcache extends FKExcel_CachedObjectStorage_CacheBase implements FKExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty) {
			...
			if (!$this->_memcache->replace($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
				if (!$this->_memcache->add($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new FKExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in MemCache');
				}
			}
	...
	/**
	 * Is a value set in the current FKExcel_CachedObjectStorage_ICache for an indexed cell?
	 *
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new FKExcel_Exception('Cell entry '.$pCoord.' no longer exists in MemCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($obj === false) {
					...
					throw new FKExcel_Exception('Cell entry '.$cellID.' no longer exists in MemCache');
				}
				if (!$this->_memcache->add($newCachePrefix.$cellID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new FKExcel_Exception('Failed to store cell '.$cellID.' in MemCache');
				}
			}
		}
	...
	public function __construct(FKExcel_Worksheet $parent, $arguments) {
		...
		if (is_null($this->_cachePrefix)) {
			...
			if (!$this->_memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, array($this, 'failureCallback'))) {
				throw new FKExcel_Exception('Could not connect to MemCache server at '.$memcacheServer.':'.$memcachePort);
			}
	...
	public function failureCallback($host, $port) {
		throw new FKExcel_Exception('memcache '.$host.':'.$port.' failed');
	}
