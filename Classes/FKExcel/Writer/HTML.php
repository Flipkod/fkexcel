 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_HTML extends FKExcel_Writer_Abstract implements FKExcel_Writer_IWriter {
	/**
	 * FKExcel object
	 *
	 * @var FKExcel
	...
	/**
	 * Create a new FKExcel_Writer_HTML
	 *
	 * @param	FKExcel	$FKExcel	FKExcel object
	 */
	public function __construct(FKExcel $FKExcel) {
		$this->_FKExcel = $FKExcel;
		$this->_defaultFont = $this->_FKExcel->getDefaultStyle()->getFont();
	}
	...
	public function save($pFilename = null) {
		// garbage collect
		$this->_FKExcel->garbageCollect();

		$saveDebugLog = FKExcel_Calculation::getInstance($this->_FKExcel)->getDebugLog()->getWriteDebugLog();
		FKExcel_Calculation::getInstance($this->_FKExcel)->getDebugLog()->setWriteDebugLog(FALSE);
		$saveArrayReturnType = FKExcel_Calculation::getArrayReturnType();
		FKExcel_Calculation::setArrayReturnType(FKExcel_Calculation::RETURN_ARRAY_AS_VALUE);
		...
		if ($fileHandle === false) {
			throw new FKExcel_Writer_Exception("Could not open file $pFilename for writing.");
		}
		...
		FKExcel_Calculation::setArrayReturnType($saveArrayReturnType);
		FKExcel_Calculation::getInstance($this->_FKExcel)->getDebugLog()->setWriteDebugLog($saveDebugLog);
	}
	...
	private function _mapVAlign($vAlign) {
		switch ($vAlign) {
			case FKExcel_Style_Alignment::VERTICAL_BOTTOM:		return 'bottom';
			case FKExcel_Style_Alignment::VERTICAL_TOP:		return 'top';
			case FKExcel_Style_Alignment::VERTICAL_CENTER:
			case FKExcel_Style_Alignment::VERTICAL_JUSTIFY:	return 'middle';
	...
	private function _mapHAlign($hAlign) {
		switch ($hAlign) {
			case FKExcel_Style_Alignment::HORIZONTAL_GENERAL:				return false;
			case FKExcel_Style_Alignment::HORIZONTAL_LEFT:					return 'left';
			case FKExcel_Style_Alignment::HORIZONTAL_RIGHT:				return 'right';
			case FKExcel_Style_Alignment::HORIZONTAL_CENTER:
			case FKExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS:	return 'center';
			case FKExcel_Style_Alignment::HORIZONTAL_JUSTIFY:				return 'justify';
	...
	private function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
			case FKExcel_Style_Border::BORDER_NONE:				return 'none';
			case FKExcel_Style_Border::BORDER_DASHDOT:				return '1px dashed';
			case FKExcel_Style_Border::BORDER_DASHDOTDOT:			return '1px dotted';
			case FKExcel_Style_Border::BORDER_DASHED:				return '1px dashed';
			case FKExcel_Style_Border::BORDER_DOTTED:				return '1px dotted';
			case FKExcel_Style_Border::BORDER_DOUBLE:				return '3px double';
			case FKExcel_Style_Border::BORDER_HAIR:				return '1px solid';
			case FKExcel_Style_Border::BORDER_MEDIUM:				return '2px solid';
			case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT:		return '2px dashed';
			case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT:	return '2px dotted';
			case FKExcel_Style_Border::BORDER_MEDIUMDASHED:		return '2px dashed';
			case FKExcel_Style_Border::BORDER_SLANTDASHDOT:		return '2px dashed';
			case FKExcel_Style_Border::BORDER_THICK:				return '3px solid';
			case FKExcel_Style_Border::BORDER_THIN:				return '1px solid';
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		// FKExcel object known?
		if (is_null($this->_FKExcel)) {
			throw new FKExcel_Writer_Exception('Internal FKExcel object not set to an instance of an object.');
		}

		// Construct HTML
		$properties = $this->_FKExcel->getProperties();
		$html = '<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">' . PHP_EOL;
		$html .= '<!-- Generated by FKExcel - http://www.FKExcel.net -->' . PHP_EOL;
		$html .= '<html>' . PHP_EOL;
		$html .= '  <head>' . PHP_EOL;
	...
	public function generateSheetData() {
		...
		if (is_null($this->_sheetIndex)) {
			$sheets = $this->_FKExcel->getAllSheets();
		} else {
			$sheets[] = $this->_FKExcel->getSheet($this->_sheetIndex);
		}
		...
		foreach ($sheets as $sheet) {
			...
			$dimension[0] = FKExcel_Cell::coordinateFromString($dimension[0]);
			$dimension[0][0] = FKExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = FKExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = FKExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;
			...
			// Writing PDF?
			if ($this->_isPdf) {
				if (is_null($this->_sheetIndex) && $sheetId + 1 < $this->_FKExcel->getSheetCount()) {
	...
	private function _extendRowsForChartsAndImages(FKExcel_Worksheet $pSheet, $row) {
		$rowMax = $row;
		$colMax = 'A';
		if ($this->_includeCharts) {
			foreach ($pSheet->getChartCollection() as $chart) {
				if ($chart instanceof FKExcel_Chart) {
				    $chartCoordinates = $chart->getTopLeftPosition();
				    $chartTL = FKExcel_Cell::coordinateFromString($chartCoordinates['cell']);
					$chartCol = FKExcel_Cell::columnIndexFromString($chartTL[0]);
					if ($chartTL[1] > $rowMax) {
						$rowMax = $chartTL[1];
						if ($chartCol > FKExcel_Cell::columnIndexFromString($colMax)) {
							$colMax = $chartTL[0];
						}
					}
				}
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof FKExcel_Worksheet_Drawing) {
			    $imageTL = FKExcel_Cell::coordinateFromString($drawing->getCoordinates());
				$imageCol = FKExcel_Cell::columnIndexFromString($imageTL[0]);
				if ($imageTL[1] > $rowMax) {
					$rowMax = $imageTL[1];
					if ($imageCol > FKExcel_Cell::columnIndexFromString($colMax)) {
						$colMax = $imageTL[0];
					}
				}
			}
	 ...
	 *
	 * @param	FKExcel_Worksheet	$pSheet			FKExcel_Worksheet
	 * @param	string				$coordinates	Cell coordinates
	 * @return	string
	 * @throws	FKExcel_Writer_Exception
	 */
	private function _writeImageInCell(FKExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @throws	FKExcel_Writer_Exception
	 */
	private function _writeChartInCell(FKExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
		...
		foreach ($pSheet->getChartCollection() as $chart) {
			if ($chart instanceof FKExcel_Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					$chartFileName = FKExcel_Shared_File::sys_get_temp_dir().'/'.uniqid().'.png';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($this->_FKExcel->getCellXfCollection() as $index => $style) {
		...
		foreach ($sheets as $sheet) {
			...
			$highestColumnIndex = FKExcel_Cell::columnIndexFromString($sheet->getHighestColumn()) - 1;
			$column = -1;
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = FKExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					$width = FKExcel_Shared_Drawing::pixelsToPoints($width);
					$column = FKExcel_Cell::columnIndexFromString($columnDimension->getColumnIndex()) - 1;
			...
			if ($rowDimension->getRowHeight() == -1) {
				$pt_height = FKExcel_Shared_Font::getDefaultRowHeightByFont($this->_FKExcel->getDefaultStyle()->getFont());
			} else {
	 ...
	 * Create CSS style
	 *
	 * @param	FKExcel_Style		$pStyle			FKExcel_Style
	 * @return	array
	 */
	private function _createCSSStyle(FKExcel_Style $pStyle) {
		// Construct CSS
		$css = '';
	...
	/**
	 * Create CSS style (FKExcel_Style_Alignment)
	 *
	 * @param	FKExcel_Style_Alignment		$pStyle			FKExcel_Style_Alignment
	 * @return	array
	 */
	private function _createCSSStyleAlignment(FKExcel_Style_Alignment $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (FKExcel_Style_Font)
	 *
	 * @param	FKExcel_Style_Font		$pStyle			FKExcel_Style_Font
	 * @return	array
	 */
	private function _createCSSStyleFont(FKExcel_Style_Font $pStyle) {
		// Construct CSS
		$css = array();
		...
		if ($pStyle->getUnderline() != FKExcel_Style_Font::UNDERLINE_NONE && $pStyle->getStrikethrough()) {
			$css['text-decoration'] = 'underline line-through';
		} else if ($pStyle->getUnderline() != FKExcel_Style_Font::UNDERLINE_NONE) {
	...
	/**
	 * Create CSS style (FKExcel_Style_Borders)
	 *
	 * @param	FKExcel_Style_Borders		$pStyle			FKExcel_Style_Borders
	 * @return	array
	 */
	private function _createCSSStyleBorders(FKExcel_Style_Borders $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (FKExcel_Style_Border)
	 *
	 * @param	FKExcel_Style_Border		$pStyle			FKExcel_Style_Border
	 * @return	string
	 */
	private function _createCSSStyleBorder(FKExcel_Style_Border $pStyle) {
		// Create CSS
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	/**
	 * Create CSS style (FKExcel_Style_Fill)
	 *
	 * @param	FKExcel_Style_Fill		$pStyle			FKExcel_Style_Fill
	 * @return	array
	 */
	private function _createCSSStyleFill(FKExcel_Style_Fill $pStyle) {
		// Construct HTML
		$css = array();

		// Create CSS
		$value = $pStyle->getFillType() == FKExcel_Style_Fill::FILL_NONE ?
	 ...
	 *
	 * @param	FKExcel_Worksheet	$pSheet		The worksheet for the table we are writing
	 * @return	string
	 * @throws	FKExcel_Writer_Exception
	 */
	private function _generateTableHeader($pSheet) {
		...
		// Write <col> elements
		$highestColumnIndex = FKExcel_Cell::columnIndexFromString($pSheet->getHighestColumn()) - 1;
		$i = -1;
	 ...
	 * @throws	FKExcel_Writer_Exception
	 */
	private function _generateRow(FKExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			foreach ($pValues as $cell) {
				$coordinate = FKExcel_Cell::stringFromColumnIndex($colNum) . ($pRow + 1);
				...
				// FKExcel_Cell
				if ($cell instanceof FKExcel_Cell) {
					$cellData = '';
					...
					if ($cell->getValue() instanceof FKExcel_RichText) {
						...
						if ($this->_preCalculateFormulas) {
							$cellData = FKExcel_Style_NumberFormat::toFormattedString(
					...
					if (!$this->_useInlineCss) {
						...
						if ($sharedStyle->getAlignment()->getHorizontal() == FKExcel_Style_Alignment::HORIZONTAL_GENERAL
				...
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					$endCellCoord = FKExcel_Cell::stringFromColumnIndex($colNum + $colSpan - 1) . ($pRow + $rowSpan);
					if (!$this->_useInlineCss) {
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							//** Necessary redundant code for the sake of FKExcel_Writer_PDF **
	...
	private function _calculateSpans()
	{
		...
		$sheetIndexes = $this->_sheetIndex !== null ?
			array($this->_sheetIndex) : range(0, $this->_FKExcel->getSheetCount() - 1);

		foreach ($sheetIndexes as $sheetIndex) {
			$sheet = $this->_FKExcel->getSheet($sheetIndex);

			$candidateSpannedRow  = array();

			// loop through all Excel merged cells
			foreach ($sheet->getMergeCells() as $cells) {
				list($cells, ) = FKExcel_Cell::splitRange($cells);
				$first = $cells[0];
				$last  = $cells[1];

				list($fc, $fr) = FKExcel_Cell::coordinateFromString($first);
				$fc = FKExcel_Cell::columnIndexFromString($fc) - 1;

				list($lc, $lr) = FKExcel_Cell::coordinateFromString($last);
				$lc = FKExcel_Cell::columnIndexFromString($lc) - 1;
			...
			$countColumns = FKExcel_Cell::columnIndexFromString($sheet->getHighestColumn());
	...
	private function _setMargins(FKExcel_Worksheet $pSheet) {
		$htmlPage = '@page { ';
		$htmlBody = 'body { ';

		$left = FKExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
		$htmlPage .= 'left-margin: ' . $left;
		$htmlBody .= 'left-margin: ' . $left;
		$right = FKExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
		$htmlPage .= 'right-margin: ' . $right;
		$htmlBody .= 'right-margin: ' . $right;
		$top = FKExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
		$htmlPage .= 'top-margin: ' . $top;
		$htmlBody .= 'top-margin: ' . $top;
		$bottom = FKExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
