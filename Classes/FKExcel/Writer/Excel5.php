...
/**
 * FKExcel_Writer_Excel5
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel5 extends FKExcel_Writer_Abstract implements FKExcel_Writer_IWriter
{
	/**
	 * FKExcel object
	 *
	...
	/**
	 * Create a new FKExcel_Writer_Excel5
	 *
	...
	public function __construct(FKExcel $FKExcel) {
		...
		$this->_parser		= new FKExcel_Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$this->_writerWorkbook = new FKExcel_Writer_Excel5_Workbook($this->_FKExcel,
																	 $this->_str_total, $this->_str_unique, $this->_str_table,
																	 $this->_colors, $this->_parser);

		// Initialise worksheet writers
		$countSheets = $this->_FKExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new FKExcel_Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
																			   $this->_str_table, $this->_colors,
																			   $this->_parser,
																			   $this->_preCalculateFormulas,
																			   $this->_FKExcel->getSheet($i));
		}
		...
		$cellXfCollection = $this->_FKExcel->getCellXfCollection();
		for ($i = 0; $i < 15; ++$i) {
			$this->_writerWorkbook->addXfWriter($cellXfCollection[0], true);
		}

		// add all the cell Xfs
		foreach ($this->_FKExcel->getCellXfCollection() as $style) {
		...
		$OLE = new FKExcel_Shared_OLE_PPS_File(FKExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new FKExcel_Shared_OLE_PPS_File(FKExcel_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new FKExcel_Shared_OLE_PPS_File(FKExcel_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new FKExcel_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_FKExcel->getAllsheets() as $sheet) {
			// sheet index
			...
			$escher = new FKExcel_Shared_Escher();

			// dgContainer
			$dgContainer = new FKExcel_Shared_Escher_DgContainer();
			...
			// spgrContainer
			$spgrContainer = new FKExcel_Shared_Escher_DgContainer_SpgrContainer();
			$dgContainer->setSpgrContainer($spgrContainer);

			// add one shape which is the group shape
			$spContainer = new FKExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = FKExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = FKExcel_Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new FKExcel_Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = FKExcel_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = FKExcel_Cell::stringFromColumnIndex(FKExcel_Cell::stringFromColumnIndex($iInc - 1));
	...
	private function _buildWorkbookEscher()
	{
		...
		// dggContainer
		$dggContainer = new FKExcel_Shared_Escher_DggContainer();
			...
			if (count($sheet->getDrawingCollection()) > 0) {
				...
				foreach ($sheet->getDrawingCollection() as $drawing) {
					...
					$spId = $sheetCountShapes
						| ($this->_FKExcel->getIndex($sheet) + 1) << 10;
		...
		// bstoreContainer
		$bstoreContainer = new FKExcel_Shared_Escher_DggContainer_BstoreContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof FKExcel_Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(FKExcel_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
					...
					$blip = new FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
					$blip->setData($blipData);

					$BSE = new FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE();
					$BSE->setBlipType($blipType);
					$BSE->setBlip($blip);

					$bstoreContainer->addBSE($BSE);

				} else if ($drawing instanceof FKExcel_Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case FKExcel_Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case FKExcel_Worksheet_MemoryDrawing::RENDERING_GIF:
					case FKExcel_Worksheet_MemoryDrawing::RENDERING_PNG:
					case FKExcel_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeDocumentSummaryInformation(){
		...
		if($this->_FKExcel->getProperties()->getCategory()){
			$dataProp = $this->_FKExcel->getProperties()->getCategory();
	...
	private function _writeSummaryInformation(){
		...
		//	Title
		if($this->_FKExcel->getProperties()->getTitle()){
			$dataProp = $this->_FKExcel->getProperties()->getTitle();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x02),
								   'offset' => array('pack' => 'V'),
		...
		//	Subject
		if($this->_FKExcel->getProperties()->getSubject()){
			$dataProp = $this->_FKExcel->getProperties()->getSubject();
		...
		//	Author (Creator)
		if($this->_FKExcel->getProperties()->getCreator()){
			$dataProp = $this->_FKExcel->getProperties()->getCreator();
		...
		//	Keywords
		if($this->_FKExcel->getProperties()->getKeywords()){
			$dataProp = $this->_FKExcel->getProperties()->getKeywords();
		...
		if($this->_FKExcel->getProperties()->getDescription()){
			$dataProp = $this->_FKExcel->getProperties()->getDescription();
		...
		if($this->_FKExcel->getProperties()->getLastModifiedBy()){
			$dataProp = $this->_FKExcel->getProperties()->getLastModifiedBy();
		...
		//	Created Date/Time
		if($this->_FKExcel->getProperties()->getCreated()){
			$dataProp = $this->_FKExcel->getProperties()->getCreated();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   'offset' => array('pack' => 'V'),
								   ...
								   'data'	=> array('data' => FKExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
		if($this->_FKExcel->getProperties()->getModified()){
			$dataProp = $this->_FKExcel->getProperties()->getModified();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0D),
								   'offset' => array('pack' => 'V'),
								   'type' 	=> array('pack' => 'V', 'data' => 0x40), // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
								   'data'	=> array('data' => FKExcel_Shared_OLE::LocalDate2OLE($dataProp)));
