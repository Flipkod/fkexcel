 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel5_Escher
{
	/**
	...
	public function close()
	{
		...
		case 'FKExcel_Shared_Escher':
			if ($dggContainer = $this->_object->getDggContainer()) {
				$writer = new FKExcel_Writer_Excel5_Escher($dggContainer);
				$this->_data = $writer->close();
			} else if ($dgContainer = $this->_object->getDgContainer()) {
				$writer = new FKExcel_Writer_Excel5_Escher($dgContainer);
				$this->_data = $writer->close();
		...
		case 'FKExcel_Shared_Escher_DggContainer':
			...
			if ($bstoreContainer = $this->_object->getBstoreContainer()) {
				$writer = new FKExcel_Writer_Excel5_Escher($bstoreContainer);
		...
		case 'FKExcel_Shared_Escher_DggContainer_BstoreContainer':
			// this is a container record
			...
			if ($BSECollection = $this->_object->getBSECollection()) {
				foreach ($BSECollection as $BSE) {
					$writer = new FKExcel_Writer_Excel5_Escher($BSE);
					$innerData .= $writer->close();
				}
			}
		...
		case 'FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE':
			...
			if ($blip = $this->_object->getBlip()) {
				$writer = new FKExcel_Writer_Excel5_Escher($blip);
				$innerData .= $writer->close();
			}
		...
		case 'FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip':
		...
		case 'FKExcel_Shared_Escher_DgContainer':
			// this is a container record
			...
			if ($spgrContainer = $this->_object->getSpgrContainer()) {
				$writer = new FKExcel_Writer_Excel5_Escher($spgrContainer);
				$innerData .= $writer->close();
		...
		case 'FKExcel_Shared_Escher_DgContainer_SpgrContainer':
			// this is a container record
			...
			foreach ($this->_object->getChildren() as $spContainer) {
				$writer = new FKExcel_Writer_Excel5_Escher($spContainer);
				$spData = $writer->close();
				$innerData .= $spData;
		...
		case 'FKExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer':
			// initialize
			$data = '';
			...
			if ($this->_object->getStartCoordinates()) {
				...
				// start coordinates
				list($column, $row) = FKExcel_Cell::coordinateFromString($this->_object->getStartCoordinates());
				$c1 = FKExcel_Cell::columnIndexFromString($column) - 1;
				$r1 = $row - 1;
				...
				// end coordinates
				list($column, $row) = FKExcel_Cell::coordinateFromString($this->_object->getEndCoordinates());
				$c2 = FKExcel_Cell::columnIndexFromString($column) - 1;
				$r2 = $row - 1;
