...
/**
 * FKExcel_Writer_Excel5_Parser
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel5_Parser
{
	/**	Constants				*/
	...
	function _convert($token)
	{
		...
		throw new FKExcel_Writer_Exception("Unknown token $token");
	}
	...
	function _convertNumber($num)
	{
		...
		if ((preg_match("/^\d+$/", $num)) and ($num <= 65535)) {
			...
			if (FKExcel_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
				$num = strrev($num);
			}
	...
	function _convertString($string)
	{
		...
		if (strlen($string) > 255) {
			throw new FKExcel_Writer_Exception("String is too long");
		}

		return pack('C', $this->ptg['ptgStr']) . FKExcel_Shared_String::UTF8toBIFF8UnicodeShort($string);
	}
...
//		$volatile = $this->_functions[$token][3];
	...
	function _convertRange2d($range, $class=0)
	{
		...
		if (preg_match('/^(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)\:(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)$/', $range)) {
			...
			// TODO: use real error codes
			throw new FKExcel_Writer_Exception("Unknown range separator");
		}
		...
		if ($class == 0) {
			...
			// TODO: use real error codes
			throw new FKExcel_Writer_Exception("Unknown class $class");
		}
...
//		}
	...
	function _packExtRef($ext_ref)
	{
		...
		if (preg_match("/:/", $ext_ref)) {
			...
			if ($sheet1 == -1) {
				throw new FKExcel_Writer_Exception("Unknown sheet name $sheet_name1 in formula");
			}
			$sheet2 = $this->_getSheetIndex($sheet_name2);
			if ($sheet2 == -1) {
				throw new FKExcel_Writer_Exception("Unknown sheet name $sheet_name2 in formula");
			}
			...
			if ($sheet1 == -1) {
				throw new FKExcel_Writer_Exception("Unknown sheet name $ext_ref in formula");
			}
			$sheet2 = $sheet1;
		}
	 ...
	 * FKExcel_Writer_Excel5_Workbook class.
	 *
	 * @access	private
	 ...
	 *
	 * @access public
	 * @see FKExcel_Writer_Excel5_Workbook::addWorksheet()
	...
	function _cellToPackedRowcol($cell)
	{
		...
		if ($col >= 256) {
			throw new FKExcel_Writer_Exception("Column in: $cell greater than 255");
		}
		if ($row >= 65536) {
			throw new FKExcel_Writer_Exception("Row in: $cell greater than 65536 ");
		}
	...
	function _rangeToPackedRange($range)
	{
		...
		if (($row1 >= 65536) or ($row2 >= 65536)) {
			throw new FKExcel_Writer_Exception("Row in: $range greater than 65536 ");
		}
	...
	function _fact()
	{
		if ($this->_current_token == "(") {
			...
			if ($this->_current_token != ")") {
				throw new FKExcel_Writer_Exception("')' token expected.");
			}
			$this->_advance();         // eat the ")"
			return $result;
		}
		...
		throw new FKExcel_Writer_Exception("Syntax error: ".$this->_current_token.
	...
	function _func()
	{
		...
		/**/
			if ($num_args > 0) {
					...
					$this->_current_token == ";")
				{
					...
					throw new FKExcel_Writer_Exception("Syntax error: comma expected in ".
		...
		if (!isset($this->_functions[$function])) {
			throw new FKExcel_Writer_Exception("Function $function() doesn't exist");
		}
		$args = $this->_functions[$function][1];
		// If fixed number of args eg. TIME($i,$j,$k). Check that the number of args is valid.
		if (($args >= 0) and ($args != $num_args)) {
			throw new FKExcel_Writer_Exception("Incorrect number of arguments in function $function() ");
		}
