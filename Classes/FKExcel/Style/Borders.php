...
/**
 * FKExcel_Style_Borders
 *
 * @category   FKExcel
 * @package    FKExcel_Style
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Style_Borders extends FKExcel_Style_Supervisor implements FKExcel_IComparable
{
	/* Diagonal directions */
	...
	/**
     * Create a new FKExcel_Style_Borders
	 *
    ...
    public function __construct($isSupervisor = FALSE, $isConditional = FALSE)
    {
    	...
    	// Initialise values
    	$this->_left				= new FKExcel_Style_Border($isSupervisor, $isConditional);
    	$this->_right				= new FKExcel_Style_Border($isSupervisor, $isConditional);
    	$this->_top					= new FKExcel_Style_Border($isSupervisor, $isConditional);
    	$this->_bottom				= new FKExcel_Style_Border($isSupervisor, $isConditional);
    	$this->_diagonal			= new FKExcel_Style_Border($isSupervisor, $isConditional);
		$this->_diagonalDirection	= FKExcel_Style_Borders::DIAGONAL_NONE;

		// Specially for supervisor
		if ($isSupervisor) {
			// Initialize pseudo-borders
			$this->_allBorders			= new FKExcel_Style_Border(TRUE);
			$this->_outline				= new FKExcel_Style_Border(TRUE);
			$this->_inside				= new FKExcel_Style_Border(TRUE);
			$this->_vertical			= new FKExcel_Style_Border(TRUE);
			$this->_horizontal			= new FKExcel_Style_Border(TRUE);
     ...
     *
     * <code>
     * $objFKExcel->getActiveSheet()->getStyle('B2')->getBorders()->applyFromArray(
     * 		array(
     * 			'bottom'     => array(
    ...
    public function getAllBorders() {
		if (!$this->_isSupervisor) {
			throw new FKExcel_Exception('Can only get pseudo-border for supervisor.');
		}
     ...
     * @return FKExcel_Style_Borders
     */
    public function setDiagonalDirection($pValue = FKExcel_Style_Borders::DIAGONAL_NONE) {
        if ($pValue == '') {
    		$pValue = FKExcel_Style_Borders::DIAGONAL_NONE;
    	}
		if ($this->_isSupervisor) {
