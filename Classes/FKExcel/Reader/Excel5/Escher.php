...
/**
 * FKExcel_Reader_Excel5_Escher
 *
 * @category   FKExcel
 * @package    FKExcel_Reader_Excel5
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_Excel5_Escher
{
	...
	/**
	 * Create a new FKExcel_Reader_Excel5_Escher instance
	 *
	 * @param mixed $object
	 */
	...
	public function load($data)
	{
		...
		while ($this->_pos < $this->_dataSize) {
			...
			$fbt = FKExcel_Reader_Excel5::_GetInt2d($this->_data, $this->_pos + 2);
	...
	private function _readDefault()
	{
		...
		$verInstance = FKExcel_Reader_Excel5::_GetInt2d($this->_data, $this->_pos);
		...
		$length = FKExcel_Reader_Excel5::_GetInt4d($this->_data, $this->_pos + 4);
	...
	private function _readDggContainer()
	{
		...
		$reader = new FKExcel_Reader_Excel5_Escher($dggContainer);
	...
	private function _readBstoreContainer()
	{
		...
		$reader = new FKExcel_Reader_Excel5_Escher($bstoreContainer);
		$reader->load($recordData);
	}
	...
	private function _readBSE()
	{
		...
		$recInstance = (0xFFF0 & FKExcel_Reader_Excel5::_GetInt2d($this->_data, $this->_pos)) >> 4;
		...
		$tag = FKExcel_Reader_Excel5::_GetInt2d($recordData, 18);

		// offset: 20; size: 4; size of BLIP in bytes
		$size = FKExcel_Reader_Excel5::_GetInt4d($recordData, 20);

		// offset: 24; size: 4; number of references to this BLIP
		$cRef = FKExcel_Reader_Excel5::_GetInt4d($recordData, 24);

		// offset: 28; size: 4; MSOFO file offset
		$foDelay = FKExcel_Reader_Excel5::_GetInt4d($recordData, 28);
		...
		// record is a container, read contents
		$reader = new FKExcel_Reader_Excel5_Escher($BSE);
	...
	private function _readDgContainer()
	{
		...
		$reader = new FKExcel_Reader_Excel5_Escher($dgContainer);
	...
	private function _readSpgrContainer()
	{
		...
		if ($this->_object instanceof FKExcel_Shared_Escher_DgContainer) {
			// DgContainer
		...
		$reader = new FKExcel_Reader_Excel5_Escher($spgrContainer);
		$escher = $reader->load($recordData);
	}
	...
	private function _readSpContainer()
	{
		...
		// record is a container, read contents
		$reader = new FKExcel_Reader_Excel5_Escher($spContainer);
		$escher = $reader->load($recordData);
	}
	...
	private function _readClientAnchor()
	{
		...
		$c1 = FKExcel_Reader_Excel5::_GetInt2d($recordData, 2);

		// offset: 4; size: 2; upper-left corner horizontal offset in 1/1024 of column width
		$startOffsetX = FKExcel_Reader_Excel5::_GetInt2d($recordData, 4);

		// offset: 6; size: 2; upper-left corner row index (0-based)
		$r1 = FKExcel_Reader_Excel5::_GetInt2d($recordData, 6);

		// offset: 8; size: 2; upper-left corner vertical offset in 1/256 of row height
		$startOffsetY = FKExcel_Reader_Excel5::_GetInt2d($recordData, 8);

		// offset: 10; size: 2; bottom-right corner column index (0-based)
		$c2 = FKExcel_Reader_Excel5::_GetInt2d($recordData, 10);

		// offset: 12; size: 2; bottom-right corner horizontal offset in 1/1024 of column width
		$endOffsetX = FKExcel_Reader_Excel5::_GetInt2d($recordData, 12);

		// offset: 14; size: 2; bottom-right corner row index (0-based)
		$r2 = FKExcel_Reader_Excel5::_GetInt2d($recordData, 14);

		// offset: 16; size: 2; bottom-right corner vertical offset in 1/256 of row height
		$endOffsetY = FKExcel_Reader_Excel5::_GetInt2d($recordData, 16);

		// set the start coordinates
		$this->_object->setStartCoordinates(FKExcel_Cell::stringFromColumnIndex($c1) . ($r1 + 1));
		...
		$this->_object->setEndCoordinates(FKExcel_Cell::stringFromColumnIndex($c2) . ($r2 + 1));
	...
	private function _readOfficeArtRGFOPTE($data, $n) {
		...
		for ($i = 0; $i < $n; ++$i) {
			...
			$opid = FKExcel_Reader_Excel5::_GetInt2d($fopte, 0);
			...
			$op = FKExcel_Reader_Excel5::_GetInt4d($fopte, 2);
