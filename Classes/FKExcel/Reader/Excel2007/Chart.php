 ...
 *
 * @category	FKExcel
 * @package		FKExcel_Reader_Excel2007
...
/**
 * FKExcel_Reader_Excel2007_Chart
 *
 * @category	FKExcel
 * @package		FKExcel_Reader_Excel2007
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_Excel2007_Chart
{
						...
						switch ($chartDetailsKey) {
							case "plotArea":
									...
									if ($plotAreaLayout == NULL) {
										$plotAreaLayout = new FKExcel_Chart_Layout();
									}
									$plotArea = new FKExcel_Chart_PlotArea($plotAreaLayout,$plotSeries);
							...
							case "legend":
									...
									$legend = new FKExcel_Chart_Legend($legendPos, $legendLayout, $legendOverlay);
									break;
						}
					}
			}
		}
		$chart = new FKExcel_Chart($chartName,$title,$legend,$plotArea,$plotVisOnly,$dispBlanksAs,$XaxisLabel,$YaxisLabel);
	...
	private static function _chartTitle($titleDetails,$namespacesChartMeta,$type) {
		...
		return new FKExcel_Chart_Title($caption, $titleLayout);
...
//			echo $detailKey,' => ',self::_getAttribute($detail, 'val', 'string'),PHP_EOL;
		...
		return new FKExcel_Chart_Layout($layout);
	...
	private static function _chartDataSeries($chartDetail,$namespacesChartMeta,$plotType) {
		...
		return new FKExcel_Chart_DataSeries($plotType,$multiSeriesType,$plotOrder,$seriesLabel,$seriesCategory,$seriesValues,$smoothLine);
	...
	private static function _chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta, $marker = null, $smoothLine = false) {
		if (isset($seriesDetail->strRef)) {
			...
			return new FKExcel_Chart_DataSeriesValues('String',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		} elseif (isset($seriesDetail->numRef)) {
			$seriesSource = (string) $seriesDetail->numRef->f;
			$seriesData = self::_chartDataSeriesValues($seriesDetail->numRef->numCache->children($namespacesChartMeta['c']));

			return new FKExcel_Chart_DataSeriesValues('Number',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		...
		foreach($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
			...
			if (isset($titleDetailElement->rPr)) {
				...
				if (!is_null($fontColor)) {
					$objText->getFont()->setColor( new FKExcel_Style_Color( self::_readColor($fontColor) ) );
				}
				...
				if (!is_null($underscore)) {
					if ($underscore == 'sng') {
						...
						$objText->getFont()->setUnderline(FKExcel_Style_Font::UNDERLINE_DOUBLE);
					} else {
						$objText->getFont()->setUnderline(FKExcel_Style_Font::UNDERLINE_NONE);
					}
				}
