 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_CSV extends FKExcel_Reader_Abstract implements FKExcel_Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	/**
	 * Create a new FKExcel_Reader_CSV
	 */
	...
	public function loadIntoExisting($pFilename, FKExcel $objFKExcel)
	{
		...
		// Create new FKExcel object
		while ($objFKExcel->getSheetCount() <= $this->_sheetIndex) {
			$objFKExcel->createSheet();
		}
		$sheet = $objFKExcel->setActiveSheetIndex($this->_sheetIndex);
			...
			foreach($rowData as $rowDatum) {
				if ($rowDatum != '' && $this->_readFilter->readCell($columnLetter, $currentRow)) {
					...
					if ($this->_inputEncoding !== 'UTF-8') {
						$rowDatum = FKExcel_Shared_String::ConvertEncoding($rowDatum, 'UTF-8', $this->_inputEncoding);
					}
