...
/**
 * FKExcel_CachedObjectStorage_Wincache
 *
 * @category   FKExcel
 * @package	FKExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_CachedObjectStorage_Wincache extends FKExcel_CachedObjectStorage_CacheBase implements FKExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty) {
			...
			if (wincache_ucache_exists($this->_cachePrefix.$this->_currentObjectID.'.cache')) {
				if (!wincache_ucache_set($this->_cachePrefix.$this->_currentObjectID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new FKExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in WinCache');
				}
			} else {
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new FKExcel_Exception('Cell entry '.$pCoord.' no longer exists in WinCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($success === false) {
					...
					throw new FKExcel_Exception('Cell entry '.$cellID.' no longer exists in Wincache');
				}
				if (!wincache_ucache_add($newCachePrefix.$cellID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new FKExcel_Exception('Failed to store cell '.$cellID.' in Wincache');
				}
			}
		}
