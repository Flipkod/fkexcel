...
/**
 * FKExcel_CachedObjectStorage_APC
 *
 * @category   FKExcel
 * @package    FKExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_CachedObjectStorage_APC extends FKExcel_CachedObjectStorage_CacheBase implements FKExcel_CachedObjectStorage_ICache {
    ...
    protected function _storeData() {
        if ($this->_currentCellIsDirty) {
            ...
            if (!apc_store($this->_cachePrefix.$this->_currentObjectID.'.cache',serialize($this->_currentObject),$this->_cacheTime)) {
                $this->__destruct();
                throw new FKExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in APC');
            }
    ...
    public function isDataSet($pCoord) {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === FALSE) {
                ...
                throw new FKExcel_Exception('Cell entry '.$pCoord.' no longer exists in APC cache');
            }
            return true;
        }
        return false;
        ...
        foreach($cacheList as $cellID) {
            if ($cellID != $this->_currentObjectID) {
                ...
                if ($obj === FALSE) {
                    ...
                    throw new FKExcel_Exception('Cell entry '.$cellID.' no longer exists in APC');
                }
                if (!apc_store($newCachePrefix.$cellID.'.cache',$obj,$this->_cacheTime)) {
                    $this->__destruct();
                    throw new FKExcel_Exception('Failed to store cell '.$cellID.' in APC');
                }
            }
        }
