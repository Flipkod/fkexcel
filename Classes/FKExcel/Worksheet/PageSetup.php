 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Worksheet_PageSetup
{
	/* Paper size */
	 ...
	 * @var int
	 */
	private $_paperSize		= FKExcel_Worksheet_PageSetup::PAPERSIZE_LETTER;
	 ...
	 * @var string
	 */
	private $_orientation	= FKExcel_Worksheet_PageSetup::ORIENTATION_DEFAULT;
    ...
    /**
     * Create a new FKExcel_Worksheet_PageSetup
     */
	...
	public function setScale($pValue = 100, $pUpdate = true) {
		...
		if (($pValue >= 0) || is_null($pValue)) {
			...
			throw new FKExcel_Exception("Scale must not be negative");
		}
		return $this;
	}
	...
	public function getPrintArea($index = 0) {
		...
		throw new FKExcel_Exception("Requested Print Area does not exist");
	}
	...
	public function setPrintArea($value, $index = 0, $method = self::SETPRINTRANGE_OVERWRITE) {
		if (strpos($value,'!') !== false) {
			throw new FKExcel_Exception('Cell coordinate must not specify a worksheet.');
		} elseif (strpos($value,':') === false) {
			throw new FKExcel_Exception('Cell coordinate must be a range of cells.');
		} elseif (strpos($value,'$') !== false) {
		...
		if ($method == self::SETPRINTRANGE_OVERWRITE) {
			if ($index == 0) {
				...
				if (($index <= 0) || ($index > count($printAreas))) {
		    		throw new FKExcel_Exception('Invalid index for setting print range.');
				}
    		...
    		throw new FKExcel_Exception('Invalid method for setting print range.');
		}
    ...
    {
    	return $this->setPrintArea(FKExcel_Cell::stringFromColumnIndex($column1) . $row1 . ':' . FKExcel_Cell::stringFromColumnIndex($column2) . $row2, $index, $method);
    }
    ...
    {
    	return $this->setPrintArea(FKExcel_Cell::stringFromColumnIndex($column1) . $row1 . ':' . FKExcel_Cell::stringFromColumnIndex($column2) . $row2, $index, self::SETPRINTRANGE_INSERT);
	}
