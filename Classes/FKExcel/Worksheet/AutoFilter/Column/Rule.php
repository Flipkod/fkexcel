...
/**
 * FKExcel_Worksheet_AutoFilter_Column_Rule
 *
 * @category	FKExcel
 * @package		FKExcel_Worksheet
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Worksheet_AutoFilter_Column_Rule
{
...
//	const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER	= 'allDatesInQuarter';	//	<dynamicFilter type="Q2"/> for Quarter 2
	 ...
	 * Autofilter Column
	 *
	 * @var FKExcel_Worksheet_AutoFilter_Column
	 */
	...
	/**
	 * Create a new FKExcel_Worksheet_AutoFilter_Column_Rule
	 *
	 * @param FKExcel_Worksheet_AutoFilter_Column $pParent
	 */
	public function __construct(FKExcel_Worksheet_AutoFilter_Column $pParent = NULL)
	{
	...
	public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER) {
		if (!in_array($pRuleType,self::$_ruleTypes)) {
			throw new FKExcel_Exception('Invalid rule type for column AutoFilter Rule.');
		}
	...
	public function setValue($pValue = '') {
		if (is_array($pValue)) {
			...
			if (count($pValue) == 0) {
				throw new FKExcel_Exception('Invalid rule value for column AutoFilter Rule.');
			}
	...
	public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL) {
		...
		if ((!in_array($pOperator,self::$_operators)) &&
			...
			throw new FKExcel_Exception('Invalid operator for column AutoFilter Rule.');
		}
	 ...
	 *
	 * @param FKExcel_Worksheet_AutoFilter_Column
	 * @return FKExcel_Worksheet_AutoFilter_Column_Rule
	 */
	public function setParent(FKExcel_Worksheet_AutoFilter_Column $pParent = NULL) {
		$this->_parent = $pParent;
