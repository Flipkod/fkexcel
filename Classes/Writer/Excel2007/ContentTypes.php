...
/**
 * FKExcel_Writer_Excel2007_ContentTypes
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel2007_ContentTypes extends FKExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeContentTypes(FKExcel $pFKExcel = null, $includeCharts = FALSE)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				$drawings = $pFKExcel->getSheet($i)->getDrawingCollection();
				$drawingCount = count($drawings);
				$chartCount = ($includeCharts) ? $pFKExcel->getSheet($i)->getChartCount() : 0;
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pFKExcel->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof FKExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof FKExcel_Worksheet_MemoryDrawing) {
			...
			$sheetCount = $pFKExcel->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pFKExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($pFKExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (FKExcel_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new FKExcel_Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeDefaultContentType(FKExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeOverrideContentType(FKExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
