...
/**
 * FKExcel_Writer_Excel2007_Chart
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel2007_Chart extends FKExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeChart(FKExcel_Chart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * Write Chart Title
	 *
	 * @param	FKExcel_Chart_Title		$title
	 * @param 	FKExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeTitle(FKExcel_Chart_Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * Write Chart Legend
	 *
	 * @param	FKExcel_Chart_Legend		$legend
	 * @param 	FKExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeLegend(FKExcel_Chart_Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 *
	 * @param	FKExcel_Chart_PlotArea		$plotArea
	 * @param	FKExcel_Chart_Title		$xAxisLabel
	 * @param	FKExcel_Chart_Title		$yAxisLabel
	 * @param 	FKExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writePlotArea(FKExcel_Chart_PlotArea $plotArea,
									FKExcel_Chart_Title $xAxisLabel = NULL,
									FKExcel_Chart_Title $yAxisLabel = NULL,
									$objWriter,
									FKExcel_Worksheet $pSheet)
	{
		if (is_null($plotArea)) {
			return;
		}
				...
				$objWriter->startElement('c:'.$chartType);
					...
					for($i = 0; $i < $groupCount; ++$i) {
						...
						if ($groupType == $chartType) {
							...
							if ($groupType === FKExcel_Chart_DataSeries::TYPE_RADARCHART) {
								$objWriter->startElement('c:radarStyle');
									$objWriter->writeAttribute('val', $plotStyle );
								$objWriter->endElement();
							} elseif ($groupType === FKExcel_Chart_DataSeries::TYPE_SCATTERCHART) {
					...
					if ($chartType === FKExcel_Chart_DataSeries::TYPE_LINECHART) {
						//	Line only, Line3D can't be smoothed

						$objWriter->startElement('c:smooth');
							$objWriter->writeAttribute('val', (integer) $plotGroup->getSmoothLine() );
						$objWriter->endElement();
					} elseif (($chartType === FKExcel_Chart_DataSeries::TYPE_BARCHART) ||
						($chartType === FKExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
						...
						if ($plotGroupingType == 'percentStacked' ||
					...
					} elseif ($chartType === FKExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
							...
							$objWriter->startElement('c:showNegBubbles');
								$objWriter->writeAttribute('val', 0 );
							$objWriter->endElement();
					} elseif ($chartType === FKExcel_Chart_DataSeries::TYPE_STOCKCHART) {
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== FKExcel_Chart_DataSeries::TYPE_PIECHART) &&
						($chartType !== FKExcel_Chart_DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== FKExcel_Chart_DataSeries::TYPE_DONUTCHART)) {
						...
						if ($chartType === FKExcel_Chart_DataSeries::TYPE_DONUTCHART) {
			...
			if (($chartType !== FKExcel_Chart_DataSeries::TYPE_PIECHART) &&
				...
				if ($chartType === FKExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	 ...
	 *
	 * @param 	FKExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @param 	FKExcel_Chart_Layout		$chartLayout	Chart layout
	 * @throws 	FKExcel_Writer_Exception
	 */
	 ...
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeCatAx($objWriter, FKExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(FKExcel_Shared_String::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeValAx($objWriter, FKExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
					...
					if ($groupType !== FKExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	...
	private static function _getChartType($plotArea)
	{
		...
		if ($groupCount == 1) {
			...
			if (count($chartTypes) == 0) {
				throw new FKExcel_Writer_Exception('Chart is not yet implemented');
			}
		}
	 ...
	 *
	 * @param	FKExcel_Chart_DataSeries		$plotGroup
									...
									)
	{
		...
		if (($groupType == FKExcel_Chart_DataSeries::TYPE_BARCHART) ||
			($groupType == FKExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
		...
		if (($groupType !== FKExcel_Chart_DataSeries::TYPE_RADARCHART) &&
			($groupType !== FKExcel_Chart_DataSeries::TYPE_STOCKCHART)) {

			if ($groupType !== FKExcel_Chart_DataSeries::TYPE_LINECHART) {
				if (($groupType == FKExcel_Chart_DataSeries::TYPE_PIECHART) ||
					($groupType == FKExcel_Chart_DataSeries::TYPE_PIECHART_3D) ||
					($groupType == FKExcel_Chart_DataSeries::TYPE_DONUTCHART) ||
					($plotSeriesCount > 1)) {
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			$objWriter->startElement('c:ser');
				...
				if (($groupType == FKExcel_Chart_DataSeries::TYPE_LINECHART) ||
					...
					$objWriter->startElement('c:spPr');
						$objWriter->startElement('a:ln');
            				...
            				if ($groupType == FKExcel_Chart_DataSeries::TYPE_STOCKCHART) {
				...
				if (($groupType === FKExcel_Chart_DataSeries::TYPE_BARCHART) ||
					($groupType === FKExcel_Chart_DataSeries::TYPE_BARCHART_3D) ||
					($groupType === FKExcel_Chart_DataSeries::TYPE_BUBBLECHART)) {
				...
				if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
					...
					if (($groupType === FKExcel_Chart_DataSeries::TYPE_BUBBLECHART) ||
						($groupType === FKExcel_Chart_DataSeries::TYPE_SCATTERCHART)) {
	 ...
	 *
	 * @param	FKExcel_Chart_DataSeriesValues		$plotSeriesLabel
	 * @param 	FKExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @throws 	FKExcel_Writer_Exception
	 */
	 ...
	 *
	 * @param	FKExcel_Chart_DataSeriesValues		$plotSeriesValues
	 * @param 	FKExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @param	string								$groupType			Type of plot for dataseries
	 ...
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, FKExcel_Worksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * Write Layout
	 *
	 * @param	FKExcel_Chart_Layout		$layout
	 * @param 	FKExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	FKExcel_Writer_Exception
	 */
	private function _writeLayout(FKExcel_Chart_Layout $layout = NULL, $objWriter)
	{
