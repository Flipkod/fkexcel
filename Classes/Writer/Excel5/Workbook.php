...
/**
 * FKExcel_Writer_Excel5_Workbook
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel5_Workbook extends FKExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	 ...
	 * Fonts writers
	 *
	 * @var FKExcel_Writer_Excel5_Font[]
	 */
	 ...
	 * Class constructor
	 *
	 * @param FKExcel	$FKExcel		The Workbook
	 ...
	 */
	public function __construct(FKExcel $FKExcel = null,
								...
								$parser )
	{
		...
		$countSheets = $FKExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$phpSheet = $FKExcel->getSheet($i);
	...
	{
		$xfWriter = new FKExcel_Writer_Excel5_Xf($style);
	 ...
	 *
	 * @param FKExcel_Style_Font $font
	 * @return int Index to FONT record
	 */
	public function _addFont(FKExcel_Style_Font $font)
	{
		...
		if(isset($this->_addedFonts[$fontHashCode])){
			...
			$fontWriter = new FKExcel_Writer_Excel5_Font($font);
	...
	public function writeWorkbook($pWorksheetSizes = null)
	{
		...
		$total_worksheets = $this->_FKExcel->getSheetCount();
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$this->_writeBoundsheet($this->_FKExcel->getSheet($i), $this->_worksheetOffsets[$i]);
		}
	...
	function _calcSheetOffsets()
	{
		...
		$total_worksheets = count($this->_FKExcel->getAllSheets());
		foreach ($this->_FKExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + strlen(FKExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeExterns()
	{
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writeExternsheet($this->_FKExcel->getSheet($i)->getTitle());
		}
	}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$sheetSetup = $this->_FKExcel->getSheet($i)->getPageSetup();
			// Write a Name record if the print area has been defined
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = FKExcel_Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = FKExcel_Cell::coordinateFromString($printArea[0]);
				$printArea[1] = FKExcel_Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = FKExcel_Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = FKExcel_Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = FKExcel_Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = FKExcel_Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		// Named ranges
		if (count($this->_FKExcel->getNamedRanges()) > 0) {
			// Loop named ranges
			$namedRanges = $this->_FKExcel->getNamedRanges();
			foreach ($namedRanges as $namedRange) {

				// Create absolute coordinate
				$range = FKExcel_Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . FKExcel_Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = FKExcel_Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = FKExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
				...
				try {
					...
					if ($namedRange->getLocalOnly()) {
						// local scope
						$scope = $this->_FKExcel->getIndex($namedRange->getScope()) + 1;
					} else {
						// global scope
						$scope = 0;
					}
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = FKExcel_Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = FKExcel_Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = FKExcel_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = FKExcel_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$sheetAutoFilter = $this->_FKExcel->getSheet($i)->getAutoFilter();
			$autoFilterRange = $sheetAutoFilter->getRange();
			if(!empty($autoFilterRange)) {
				$rangeBounds = FKExcel_Cell::rangeBoundaries($autoFilterRange);
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = FKExcel_Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
	...
	private function _writeWindow1()
	{
		...
		// not supported by FKExcel, so there is only one selected sheet, the active
		$ctabsel   = 1;       // Number of workbook tabs selected

		$wTabRatio = 0x0258;                 // Tab to scrollbar ratio

		// not supported by FKExcel, set to 0
		$itabFirst = 0;     // 1st displayed worksheet
		$itabCur   = $this->_FKExcel->getActiveSheetIndex();    // Active worksheet
	 ...
	 *
	 * @param FKExcel_Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case FKExcel_Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case FKExcel_Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case FKExcel_Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		$data .= FKExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
	...
	private function _writeSupbookInternal()
	{
		...
		$header    = pack("vv", $record, $length);
		$data      = pack("vv", $this->_FKExcel->getSheetCount(), 0x0401);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		$numberFormatString = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (FKExcel_Shared_Date::getExcelCalendar() == FKExcel_Shared_Date::CALENDAR_MAC_1904) ?
