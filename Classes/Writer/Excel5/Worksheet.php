...
/**
 * FKExcel_Writer_Excel5_Worksheet
 *
 * @category   FKExcel
 * @package    FKExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Writer_Excel5_Worksheet extends FKExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	 * @var FKExcel_Writer_Excel5_Parser
	 */
	private $_parser;
	 ...
	 * @param FKExcel_Worksheet $phpSheet
	 */
...
//		$this->_firstRowIndex = ($minR > 65535) ? 65535 : $minR;
		...
		$this->_firstColumnIndex	= FKExcel_Cell::columnIndexFromString($minC);
		$this->_lastColumnIndex		= FKExcel_Cell::columnIndexFromString($maxC);
...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
	 ...
	 *
	 * @access public
	 * @see FKExcel_Writer_Excel5_Workbook::storeWorkbook()
	 */
	function close()
	{
		...
		if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
			$defaultWidth = FKExcel_Shared_Font::getDefaultColumnWidthByFont($_phpSheet->getParent()->getDefaultStyle()->getFont());
		}
		...
		for ($i = 0; $i <= $maxCol; ++$i) {
			...
			$columnLetter = FKExcel_Cell::stringFromColumnIndex($i);
		...
		foreach ($_phpSheet->getCellCollection() as $cellID) {
			...
			$row = $cell->getRow() - 1;
			$column = FKExcel_Cell::columnIndexFromString($cell->getColumn()) - 1;
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
			...
			$cVal = $cell->getValue();
			if ($cVal instanceof FKExcel_RichText) {
				// $this->_writeString($row, $column, $cVal->getPlainText(), $xfIndex);
				$arrcRun = array();
				$str_len = FKExcel_Shared_String::CountCharacters($cVal->getPlainText(), 'UTF-8');
				$str_pos = 0;
				$elements = $cVal->getRichTextElements();
				foreach ($elements as $element) {
					// FONT Index
					if ($element instanceof FKExcel_RichText_Run) {
					...
					// Position FROM
					$str_pos += FKExcel_Shared_String::CountCharacters($element->getText(), 'UTF-8');
				}
				$this->_writeRichTextString($row, $column, $cVal->getPlainText(), $xfIndex, $arrcRun);
			} else {
				switch ($cell->getDatatype()) {
					case FKExcel_Cell_DataType::TYPE_STRING:
					case FKExcel_Cell_DataType::TYPE_NULL:
						if ($cVal === '' || $cVal === null) {
		...
		foreach ($_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
			list($column, $row) = FKExcel_Cell::coordinateFromString($coordinate);
			...
			$this->_writeUrl($row - 1, FKExcel_Cell::columnIndexFromString($column) - 1, $url);
		}
		...
		if(!empty($arrConditionalStyles)){
			...
			foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
				foreach ($conditionalStyles as $conditional) {
					if($conditional->getConditionType() == FKExcel_Style_Conditional::CONDITION_EXPRESSION
						|| $conditional->getConditionType() == FKExcel_Style_Conditional::CONDITION_CELLIS){
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		...
		$firstCellCoordinates = FKExcel_Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
		$lastCellCoordinates  = FKExcel_Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)

		return(pack('vvvv',
			$firstCellCoordinates[1] - 1,
			$lastCellCoordinates[1] - 1,
			FKExcel_Cell::columnIndexFromString($firstCellCoordinates[0]) - 1,
			FKExcel_Cell::columnIndexFromString($lastCellCoordinates[0]) - 1
		));
	}
	...
	private function _writeRichTextString($row, $col, $str, $xfIndex, $arrcRun){
		...
		$str = FKExcel_Shared_String::UTF8toBIFF8UnicodeShort($str, $arrcRun);
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($str);
	...
	private function _writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
	{
		...
		if (isset($calculatedValue)) {
			...
			if (is_bool($calculatedValue)) {
				...
				if (array_key_exists($calculatedValue, FKExcel_Cell_DataType::getErrorCodes())) {
					// Error value
	...
	private function _writeStringRecord($stringValue)
	{
		...
		$data = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($stringValue);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = FKExcel_Shared_String::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = FKExcel_Shared_String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
	...
	private function _writeWindow2()
	{
		...
		// no support in FKExcel for selected sheet, therefore sheet is only selected if it is the active sheet
		$fSelected	  = ($this->_phpSheet === $this->_phpSheet->getParent()->getActiveSheet()) ? 1 : 0;
		$fPaged		 = 1;					 // 2
		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === FKExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
	...
	private function _writeSelection()
	{
		...
		$selectedCells = FKExcel_Cell::splitRange($this->_phpSheet->getSelectedCells());
		...
		list($colFirst, $rwFirst) = FKExcel_Cell::coordinateFromString($first);
		$colFirst = FKExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
		--$rwFirst; // base 0 row index

		list($colLast, $rwLast) = FKExcel_Cell::coordinateFromString($last);
		$colLast = FKExcel_Cell::columnIndexFromString($colLast) - 1; // base 0 column index
	...
	private function _writeMergedCells()
	{
		...
		foreach ($mergeCells as $mergeCell) {
			...
			$range = FKExcel_Cell::splitRange($mergeCell);
			list($first, $last) = $range[0];
			list($firstColumn, $firstRow) = FKExcel_Cell::coordinateFromString($first);
			list($lastColumn, $lastRow) = FKExcel_Cell::coordinateFromString($last);

			$recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, FKExcel_Cell::columnIndexFromString($firstColumn) - 1, FKExcel_Cell::columnIndexFromString($lastColumn) - 1);
	...
	private function _writeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= FKExcel_Shared_String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	private function _writePanes()
	{
		...
		if ($freezePane = $this->_phpSheet->getFreezePane()) {
			list($column, $row) = FKExcel_Cell::coordinateFromString($freezePane);
			$panes[0] = $row - 1;
			$panes[1] = FKExcel_Cell::columnIndexFromString($column) - 1;
		} else {
			// thaw panes
			return;
		}
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == FKExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeHeader()
	{
		...
		*/
		...
		$recordData = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
		$length = strlen($recordData);
	...
	private function _writeFooter()
	{
		...
		*/
		...
		$recordData = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
		$length = strlen($recordData);
	...
	private function _writeAutoFilterInfo(){
		...
		$rangeBounds = FKExcel_Cell::rangeBoundaries($this->_phpSheet->getAutoFilter()->getRange());
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			// Fetch coordinates
			$coordinates = FKExcel_Cell::coordinateFromString($cell);

			// Decide what to do by the type of break
			switch ($breakType) {
				case FKExcel_Worksheet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = FKExcel_Cell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case FKExcel_Worksheet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case FKExcel_Worksheet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_end))) {
			$width -= FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, FKExcel_Cell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
	...
	function _processBitmap($bitmap)
	{
		...
		if (!$bmp_fd) {
			throw new FKExcel_Writer_Exception("Couldn't import $bitmap");
		}
		...
		if (strlen($data) <= 0x36) {
			throw new FKExcel_Writer_Exception("$bitmap doesn't contain enough data.\n");
		}

		// The first 2 bytes are used to identify the bitmap.
		$identity = unpack("A2ident", $data);
		if ($identity['ident'] != "BM") {
			throw new FKExcel_Writer_Exception("$bitmap doesn't appear to be a valid bitmap image.\n");
		}
		...
		$data   = substr($data, 8);
		if ($width > 0xFFFF) {
			throw new FKExcel_Writer_Exception("$bitmap: largest image width supported is 65k.\n");
		}
		if ($height > 0xFFFF) {
			throw new FKExcel_Writer_Exception("$bitmap: largest image height supported is 65k.\n");
		}
		...
		if ($planes_and_bitcount[2] != 24) { // Bitcount
			throw new FKExcel_Writer_Exception("$bitmap isn't a 24bit true color bitmap.\n");
		}
		if ($planes_and_bitcount[1] != 1) {
			throw new FKExcel_Writer_Exception("$bitmap: only 1 plane supported in bitmap image.\n");
		}
		...
		if ($compression['comp'] != 0) {
			throw new FKExcel_Writer_Exception("$bitmap: compression not supported in bitmap image.\n");
		}
	 ...
	 * Set Escher object
	 *
	 * @param FKExcel_Shared_Escher $pValue
	 */
	public function setEscher(FKExcel_Shared_Escher $pValue = null)
	{
		$this->_escher = $pValue;
	}
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			$writer = new FKExcel_Writer_Excel5_Escher($this->_escher);
			$data = $writer->close();
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				switch ($type) {
					case FKExcel_Cell_DataValidation::TYPE_NONE:		$type = 0x00;	break;
					case FKExcel_Cell_DataValidation::TYPE_WHOLE:		$type = 0x01;	break;
					case FKExcel_Cell_DataValidation::TYPE_DECIMAL:	$type = 0x02;	break;
					case FKExcel_Cell_DataValidation::TYPE_LIST:		$type = 0x03;	break;
					case FKExcel_Cell_DataValidation::TYPE_DATE:		$type = 0x04;	break;
					case FKExcel_Cell_DataValidation::TYPE_TIME:		$type = 0x05;	break;
					case FKExcel_Cell_DataValidation::TYPE_TEXTLENGTH:	$type = 0x06;	break;
					case FKExcel_Cell_DataValidation::TYPE_CUSTOM:		$type = 0x07;	break;
				}
				$options |= $type << 0;
				...
				switch ($errorStyle) {
					case FKExcel_Cell_DataValidation::STYLE_STOP:			$errorStyle = 0x00;		break;
					case FKExcel_Cell_DataValidation::STYLE_WARNING:		$errorStyle = 0x01;		break;
					case FKExcel_Cell_DataValidation::STYLE_INFORMATION:	$errorStyle = 0x02;		break;
				}
				...
				switch ($operator) {
					case FKExcel_Cell_DataValidation::OPERATOR_BETWEEN: $operator = 0x00			;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN: $operator = 0x01		;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_EQUAL: $operator = 0x02				;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_NOTEQUAL: $operator = 0x03			;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_GREATERTHAN: $operator = 0x04		;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_LESSTHAN: $operator = 0x05			;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL: $operator = 0x06;	break;
					case FKExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL: $operator = 0x07	;	break;
				}
				$options |= $operator << 20;
				...
				$data .= FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($promptTitle);

				// error title
				$errorTitle = $dataValidation->getErrorTitle() !== '' ?
					$dataValidation->getErrorTitle() : chr(0);
				$data .= FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($errorTitle);

				// prompt text
				$prompt = $dataValidation->getPrompt() !== '' ?
					$dataValidation->getPrompt() : chr(0);
				$data .= FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($prompt);

				// error text
				$error = $dataValidation->getError() !== '' ?
					$dataValidation->getError() : chr(0);
				$data .= FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($error);
				...
				try {
					...
					if ($formula2 === '') {
						throw new FKExcel_Writer_Exception('No formula2');
					}
	...
	private function _writePageLayoutView(){
		...
		// The options flags that comprise $grbit
		if($this->_phpSheet->getSheetView()->getView() == FKExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT){
			$fPageLayoutView   = 1;
		} else {
			$fPageLayoutView   = 0;
		}
	...
	/**
	 * Write CFRule Record
	 * @param FKExcel_Style_Conditional $conditional
	 */
	private function _writeCFRule(FKExcel_Style_Conditional $conditional){
		...
		if($conditional->getConditionType() == FKExcel_Style_Conditional::CONDITION_EXPRESSION){
			$type = 0x02;
			$operatorType = 0x00;
		} else if($conditional->getConditionType() == FKExcel_Style_Conditional::CONDITION_CELLIS){
			$type = 0x01;

			switch ($conditional->getOperatorType()){
				case FKExcel_Style_Conditional::OPERATOR_NONE:
					$operatorType = 0x00;
					break;
				case FKExcel_Style_Conditional::OPERATOR_EQUAL:
					$operatorType = 0x03;
					break;
				case FKExcel_Style_Conditional::OPERATOR_GREATERTHAN:
					$operatorType = 0x05;
					break;
				case FKExcel_Style_Conditional::OPERATOR_GREATERTHANOREQUAL:
					$operatorType = 0x07;
					break;
				case FKExcel_Style_Conditional::OPERATOR_LESSTHAN:
					$operatorType = 0x06;
					break;
				case FKExcel_Style_Conditional::OPERATOR_LESSTHANOREQUAL:
					$operatorType = 0x08;
					break;
				case FKExcel_Style_Conditional::OPERATOR_NOTEQUAL:
					$operatorType = 0x04;
					break;
				case FKExcel_Style_Conditional::OPERATOR_BETWEEN:
					$operatorType = 0x01;
					break;
		...
		if($numConditions == 1){
			...
			$operand2 = null;
		} else if($numConditions == 2 && ($conditional->getOperatorType() == FKExcel_Style_Conditional::OPERATOR_BETWEEN)){
			$szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
		...
		// Border
		$bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == FKExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == FKExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == FKExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() == FKExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == FKExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() == FKExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == FKExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == FKExcel_Style_Border::BORDER_NONE ? 1 : 0);
		...
		if($bFormatFont == 1){
			...
			if($conditional->getStyle()->getFont()->getName() == null){
				...
				$dataBlockFont = FKExcel_Shared_String::UTF8toBIFF8UnicodeLong($conditional->getStyle()->getFont()->getName());
			}
			// Font Size
			...
			switch ($conditional->getStyle()->getFont()->getUnderline()){
				case FKExcel_Style_Font::UNDERLINE_NONE             : $dataBlockFont .= pack('C', 0x00); $fontUnderline = 0; break;
				case FKExcel_Style_Font::UNDERLINE_DOUBLE           : $dataBlockFont .= pack('C', 0x02); $fontUnderline = 0; break;
				case FKExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING : $dataBlockFont .= pack('C', 0x22); $fontUnderline = 0; break;
				case FKExcel_Style_Font::UNDERLINE_SINGLE           : $dataBlockFont .= pack('C', 0x01); $fontUnderline = 0; break;
				case FKExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING : $dataBlockFont .= pack('C', 0x21); $fontUnderline = 0; break;
		...
		if($bFormatAlign == 1){
			...
			switch ($conditional->getStyle()->getAlignment()->getHorizontal()){
				case FKExcel_Style_Alignment::HORIZONTAL_GENERAL 			: $blockAlign = 0; break;
				case FKExcel_Style_Alignment::HORIZONTAL_LEFT				: $blockAlign = 1; break;
				case FKExcel_Style_Alignment::HORIZONTAL_RIGHT				: $blockAlign = 3; break;
				case FKExcel_Style_Alignment::HORIZONTAL_CENTER			: $blockAlign = 2; break;
				case FKExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS	: $blockAlign = 6; break;
				case FKExcel_Style_Alignment::HORIZONTAL_JUSTIFY			: $blockAlign = 5; break;
			}
			...
			switch ($conditional->getStyle()->getAlignment()->getVertical()){
				case FKExcel_Style_Alignment::VERTICAL_BOTTOM 				: $blockAlign = 2 << 4; break;
				case FKExcel_Style_Alignment::VERTICAL_TOP					: $blockAlign = 0 << 4; break;
				case FKExcel_Style_Alignment::VERTICAL_CENTER				: $blockAlign = 1 << 4; break;
				case FKExcel_Style_Alignment::VERTICAL_JUSTIFY				: $blockAlign = 3 << 4; break;
			}
			$blockAlign |= 0 << 7;
		...
		if($bFormatBorder == 1){
			...
			switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()){
				case FKExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00; break;
				case FKExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01; break;
				case FKExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02; break;
				case FKExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03; break;
				case FKExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04; break;
				case FKExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05; break;
				case FKExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06; break;
				case FKExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08; break;
				case FKExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A; break;
				case FKExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C; break;
				case FKExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D; break;
			}
			switch ($conditional->getStyle()->getBorders()->getRight()->getBorderStyle()){
				case FKExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 4; break;
				case FKExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 4; break;
				case FKExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 4; break;
				case FKExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 4; break;
				case FKExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 4; break;
				case FKExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 4; break;
				case FKExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 4; break;
				case FKExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 4; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 4; break;
				case FKExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 4; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 4; break;
				case FKExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 4; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 4; break;
				case FKExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 4; break;
			}
			switch ($conditional->getStyle()->getBorders()->getTop()->getBorderStyle()){
				case FKExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 8; break;
				case FKExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 8; break;
				case FKExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 8; break;
				case FKExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 8; break;
				case FKExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 8; break;
				case FKExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 8; break;
				case FKExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 8; break;
				case FKExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 8; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 8; break;
				case FKExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 8; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 8; break;
				case FKExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 8; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 8; break;
				case FKExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 8; break;
			}
			switch ($conditional->getStyle()->getBorders()->getBottom()->getBorderStyle()){
				case FKExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 12; break;
				case FKExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 12; break;
				case FKExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 12; break;
				case FKExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 12; break;
				case FKExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 12; break;
				case FKExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 12; break;
				case FKExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 12; break;
				case FKExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 12; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 12; break;
				case FKExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 12; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 12; break;
				case FKExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 12; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 12; break;
				case FKExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 12; break;
			}
			...
			switch ($conditional->getStyle()->getBorders()->getDiagonal()->getBorderStyle()){
				case FKExcel_Style_Border::BORDER_NONE              : $blockColor |= 0x00 << 21; break;
				case FKExcel_Style_Border::BORDER_THIN              : $blockColor |= 0x01 << 21; break;
				case FKExcel_Style_Border::BORDER_MEDIUM            : $blockColor |= 0x02 << 21; break;
				case FKExcel_Style_Border::BORDER_DASHED            : $blockColor |= 0x03 << 21; break;
				case FKExcel_Style_Border::BORDER_DOTTED            : $blockColor |= 0x04 << 21; break;
				case FKExcel_Style_Border::BORDER_THICK             : $blockColor |= 0x05 << 21; break;
				case FKExcel_Style_Border::BORDER_DOUBLE            : $blockColor |= 0x06 << 21; break;
				case FKExcel_Style_Border::BORDER_HAIR              : $blockColor |= 0x07 << 21; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockColor |= 0x08 << 21; break;
				case FKExcel_Style_Border::BORDER_DASHDOT           : $blockColor |= 0x09 << 21; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockColor |= 0x0A << 21; break;
				case FKExcel_Style_Border::BORDER_DASHDOTDOT        : $blockColor |= 0x0B << 21; break;
				case FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockColor |= 0x0C << 21; break;
				case FKExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockColor |= 0x0D << 21; break;
			}
		...
		if($bFormatFill == 1){
			...
			switch ($conditional->getStyle()->getFill()->getFillType()){
				case FKExcel_Style_Fill::FILL_NONE						: $blockFillPatternStyle = 0x00; break;
				case FKExcel_Style_Fill::FILL_SOLID					: $blockFillPatternStyle = 0x01; break;
				case FKExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY		: $blockFillPatternStyle = 0x02; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKGRAY			: $blockFillPatternStyle = 0x03; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY		: $blockFillPatternStyle = 0x04; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL	: $blockFillPatternStyle = 0x05; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL		: $blockFillPatternStyle = 0x06; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKDOWN			: $blockFillPatternStyle = 0x07; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKUP			: $blockFillPatternStyle = 0x08; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKGRID			: $blockFillPatternStyle = 0x09; break;
				case FKExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS		: $blockFillPatternStyle = 0x0A; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL	: $blockFillPatternStyle = 0x0B; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL	: $blockFillPatternStyle = 0x0C; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN		: $blockFillPatternStyle = 0x0D; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTUP			: $blockFillPatternStyle = 0x0E; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTGRID		: $blockFillPatternStyle = 0x0F; break;
				case FKExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS		: $blockFillPatternStyle = 0x10; break;
				case FKExcel_Style_Fill::FILL_PATTERN_GRAY125			: $blockFillPatternStyle = 0x11; break;
				case FKExcel_Style_Fill::FILL_PATTERN_GRAY0625			: $blockFillPatternStyle = 0x12; break;
				case FKExcel_Style_Fill::FILL_GRADIENT_LINEAR			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
				case FKExcel_Style_Fill::FILL_GRADIENT_PATH			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
		...
		if($bFormatProt == 1){
			...
			if($conditional->getStyle()->getProtection()->getLocked() == FKExcel_Style_Protection::PROTECTION_PROTECTED){
				$dataBlockProtection = 1;
			}
			if($conditional->getStyle()->getProtection()->getHidden() == FKExcel_Style_Protection::PROTECTION_PROTECTED){
		...
		foreach ($this->_phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
			foreach ($conditionalStyles as $conditional) {
				if($conditional->getConditionType() == FKExcel_Style_Conditional::CONDITION_EXPRESSION
					...
					// Cells
					$arrCoord = FKExcel_Cell::coordinateFromString($cellCoordinate);
					if(!is_numeric($arrCoord[0])){
						$arrCoord[0] = FKExcel_Cell::columnIndexFromString($arrCoord[0]);
					}
