...
// http://sourceforge.net/projects/FKExcelreader/
...
/**
 * FKExcel_Reader_Excel5
 *
 * This class uses {@link http://sourceforge.net/projects/FKExcelreader/parseXL}
 *
 * @category	FKExcel
 * @package		FKExcel_Reader_Excel5
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_Excel5 extends FKExcel_Reader_Abstract implements FKExcel_Reader_IReader
{
	// ParseXL definitions
	...
	/**
	 * Create a new FKExcel_Reader_Excel5 instance
	 */
	public function __construct() {
		$this->_readFilter = new FKExcel_Reader_DefaultReadFilter();
	}


	/**
	 * Can the current FKExcel_Reader_IReader read the file?
	 *
	...
	public function canRead($pFilename)
	{
		...
		if (!file_exists($pFilename)) {
			throw new FKExcel_Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
		}

		try {
			// Use ParseXL for the hard work.
			$ole = new FKExcel_Shared_OLERead();
	...
	/**
	 * Reads names of the worksheets from a file, without parsing the whole file to a FKExcel object
	 *
	 * @param 	string 		$pFilename
	 * @throws 	FKExcel_Reader_Exception
	 */
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			$tmpInfo['lastColumnLetter'] = FKExcel_Cell::stringFromColumnIndex($tmpInfo['lastColumnIndex']);
	...
	public function load($pFilename)
	{
		...
		// Initialisations
		$this->_FKExcel = new FKExcel;
		$this->_FKExcel->removeSheetByIndex(0); // remove 1st sheet
		if (!$this->_readDataOnly) {
			$this->_FKExcel->removeCellStyleXfByIndex(0); // remove the default style
			$this->_FKExcel->removeCellXfByIndex(0); // remove the default style
		}
		...
		if (!$this->_readDataOnly) {
			...
			foreach ($this->_FKExcel->getCellXfCollection() as $objStyle) {
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new FKExcel_Shared_Escher();
			$reader = new FKExcel_Reader_Excel5_Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new FKExcel_Shared_Escher());
		...
		foreach ($this->_sheets as $sheet) {
			...
			// add sheet to FKExcel object
			$this->_phpSheet = $this->_FKExcel->createSheet();
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorksheet = new FKExcel_Shared_Escher();
				$reader = new FKExcel_Reader_Excel5_Escher($escherWorksheet);
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					list($startColumn, $startRow) = FKExcel_Cell::coordinateFromString($spContainer->getStartCoordinates());
					list($endColumn, $endRow) = FKExcel_Cell::coordinateFromString($spContainer->getEndCoordinates());
					...
					$width = FKExcel_Shared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
					$height = FKExcel_Shared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

					// calculate offsetX and offsetY of the shape
					$offsetX = $startOffsetX * FKExcel_Shared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
					$offsetY = $startOffsetY * FKExcel_Shared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
...
//							echo 'Picture Object<br />';
							...
							if ($blip = $BSE->getBlip()) {
								...
								$drawing = new FKExcel_Worksheet_MemoryDrawing();
								...
								switch ($blipType) {
									case FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
										$drawing->setRenderingFunction(FKExcel_Worksheet_MemoryDrawing::RENDERING_JPEG);
										$drawing->setMimeType(FKExcel_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
										break;

									case FKExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
										$drawing->setRenderingFunction(FKExcel_Worksheet_MemoryDrawing::RENDERING_PNG);
										$drawing->setMimeType(FKExcel_Worksheet_MemoryDrawing::MIMETYPE_PNG);
										break;
								}
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					list($column, $row) = FKExcel_Cell::coordinateFromString($cell);
					if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, FKExcel_Cell_DataType::TYPE_FORMULA);
					}
				}
			}
		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					if ($docSheet = $this->_FKExcel->getSheetByName($sheetName)) {
				...
				case pack('C', 0x07):
					...
					foreach ($ranges as $range) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_FKExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = FKExcel_Cell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = FKExcel_Cell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2) {
					if (($docSheet = $this->_FKExcel->getSheetByName($explodes[0])) ||
						($docSheet = $this->_FKExcel->getSheetByName(trim($explodes[0],"'")))) {
						$extractedRange = $explodes[1];
						$extractedRange = str_replace('$', '', $extractedRange);
						...
						$scope = ($definedName['scope'] == 0) ?
							null : $this->_FKExcel->getSheetByName($this->_sheets[$definedName['scope'] - 1]['name']);

						$this->_FKExcel->addNamedRange( new FKExcel_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope) );
					}
				} else {
					//	Named Value
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = FKExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;

				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = FKExcel_Shared_OLE::OLE2LocalDate(substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				case 0x01:	//	Code Page
					$codePage = FKExcel_Shared_CodePage::NumberToName($value);
					break;

				case 0x02:	//	Title
					$this->_FKExcel->getProperties()->setTitle($value);
					break;

				case 0x03:	//	Subject
					$this->_FKExcel->getProperties()->setSubject($value);
					break;

				case 0x04:	//	Author (Creator)
					$this->_FKExcel->getProperties()->setCreator($value);
					break;

				case 0x05:	//	Keywords
					$this->_FKExcel->getProperties()->setKeywords($value);
					break;

				case 0x06:	//	Comments (Description)
					$this->_FKExcel->getProperties()->setDescription($value);
					break;

				case 0x07:	//	Template
					//	Not supported by FKExcel
					break;

				case 0x08:	//	Last Saved By (LastModifiedBy)
					$this->_FKExcel->getProperties()->setLastModifiedBy($value);
					break;
				...
				case 0x0C:	//	Created Date/Time
					$this->_FKExcel->getProperties()->setCreated($value);
					break;

				case 0x0D:	//	Modified Date/Time
					$this->_FKExcel->getProperties()->setModified($value);
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = FKExcel_Shared_OLE::OLE2LocalDate(substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				...
				case 0x02:	//	Category
					$this->_FKExcel->getProperties()->setCategory($value);
					break;
				...
				case 0x0E:	//	Manager
					$this->_FKExcel->getProperties()->setManager($value);
					break;

				case 0x0F:	//	Company
					$this->_FKExcel->getProperties()->setCompany($value);
					break;
...
//		echo '<br />';
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
					throw new FKExcel_Reader_Exception('Cannot read this Excel file. Version is too old.');
				}
	...
	private function _readFilepass()
	{
		...
		throw new FKExcel_Reader_Exception('Cannot read encrypted file');
	}
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = FKExcel_Shared_CodePage::NumberToName($codepage);
	}
	...
	private function _readDateMode()
	{
		...
		FKExcel_Shared_Date::setExcelCalendar(FKExcel_Shared_Date::CALENDAR_WINDOWS_1900);
		if (ord($recordData{0}) == 1) {
			FKExcel_Shared_Date::setExcelCalendar(FKExcel_Shared_Date::CALENDAR_MAC_1904);
		}
	}
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			$objFont = new FKExcel_Style_Font();
			...
			switch ($underlineType) {
				case 0x00:
					break; // no underline
				case 0x01:
					$objFont->setUnderline(FKExcel_Style_Font::UNDERLINE_SINGLE);
					break;
				case 0x02:
					$objFont->setUnderline(FKExcel_Style_Font::UNDERLINE_DOUBLE);
					break;
				case 0x21:
					$objFont->setUnderline(FKExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING);
					break;
				case 0x22:
					$objFont->setUnderline(FKExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
					break;
			}
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			} elseif (($code = FKExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
			...
			$objStyle->getProtection()->setLocked($isLocked ?
				FKExcel_Style_Protection::PROTECTION_INHERIT : FKExcel_Style_Protection::PROTECTION_UNPROTECTED);

			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$objStyle->getProtection()->setHidden($isHidden ?
				FKExcel_Style_Protection::PROTECTION_PROTECTED : FKExcel_Style_Protection::PROTECTION_UNPROTECTED);
			...
			switch ($horAlign) {
				case 0:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_GENERAL);
					break;
				case 1:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 2:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 3:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 4:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_FILL);
					break;
				case 5:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				case 6:
					$objStyle->getAlignment()->setHorizontal(FKExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
					break;
			}
			...
			switch ($vertAlign) {
				case 0:
					$objStyle->getAlignment()->setVertical(FKExcel_Style_Alignment::VERTICAL_TOP);
					break;
				case 1:
					$objStyle->getAlignment()->setVertical(FKExcel_Style_Alignment::VERTICAL_CENTER);
					break;
				case 2:
					$objStyle->getAlignment()->setVertical(FKExcel_Style_Alignment::VERTICAL_BOTTOM);
					break;
				case 3:
					$objStyle->getAlignment()->setVertical(FKExcel_Style_Alignment::VERTICAL_JUSTIFY);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell border lines and background area
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_BOTH);
					}
			...
			if ($isCellStyleXf) {
				...
				if ($this->_xfIndex == 0) {
					$this->_FKExcel->addCellStyleXf($objStyle);
					$this->_mapCellStyleXfIndex[$this->_xfIndex] = 0;
				}
			} else {
				// we read all cell XF records
				$this->_FKExcel->addCellXf($objStyle);
				$this->_mapCellXfIndex[$this->_xfIndex] = count($this->_FKExcel->getCellXfCollection()) - 1;
			}
				...
				switch ($extType) {
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$fill = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFill();
					...
					case 7:		// border color top
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$top = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getTop();
					...
					case 8:		// border color bottom
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$bottom = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getBottom();
					...
					case 9:		// border color left
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$left = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getLeft();
					...
					case 10:		// border color right
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$right = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getRight();
					...
					case 11:		// border color diagonal
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$diagonal = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getDiagonal();
					...
					case 13:	// font color
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$font = $this->_FKExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFont();
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isBuiltIn) {
				...
				// user-defined; not supported by FKExcel
			}
		}
	}
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = FKExcel_Worksheet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = FKExcel_Worksheet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = FKExcel_Worksheet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = FKExcel_Worksheet::SHEETSTATE_VISIBLE;      break;
		}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, FKExcel_Worksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, FKExcel_Worksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(FKExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(FKExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
	...
	private function _readRk()
	{
		...
		$columnString = FKExcel_Cell::stringFromColumnIndex($column);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell
			$cell->setValueExplicit($numValue, FKExcel_Cell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new FKExcel_RichText();
				$charPos = 0;
				$sstCount = count($this->_sst[$index]['fmtRuns']);
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = FKExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = FKExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, FKExcel_Shared_String::CountCharacters($this->_sst[$index]['value']));
					}

					if (FKExcel_Shared_String::CountCharacters($text) > 0) {
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($richText, FKExcel_Cell_DataType::TYPE_STRING);
			} else {
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], FKExcel_Cell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = FKExcel_Cell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readFormula()
	{
		...
		if ($isPartOfSharedFormula) {
			...
			$this->_baseCell = FKExcel_Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
		}
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = FKExcel_Cell_DataType::TYPE_STRING;
				...
				$dataType = FKExcel_Cell_DataType::TYPE_BOOL;
				...
				$dataType = FKExcel_Cell_DataType::TYPE_ERROR;
				...
				$dataType = FKExcel_Cell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = FKExcel_Cell_DataType::TYPE_NUMERIC;
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					if ($this->_version != self::XLS_BIFF8) {
						throw new FKExcel_Reader_Exception('Not BIFF8. Can only read BIFF8 formulas');
					}
					$formula = $this->_getFormulaFromStructure($formulaStructure); // get formula in human language
					$cell->setValueExplicit('=' . $formula, FKExcel_Cell_DataType::TYPE_FORMULA);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			switch ($isError) {
				case 0: // boolean
					...
					// add cell value
					$cell->setValueExplicit($value, FKExcel_Cell_DataType::TYPE_BOOL);
					break;
				...
				case 1: // error type
					...
					// add cell value
					$cell->setValueExplicit($value, FKExcel_Cell_DataType::TYPE_ERROR);
					break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = FKExcel_Cell::stringFromColumnIndex($fc + $i);
	...
	private function _readBlank()
	{
		...
		$columnString = FKExcel_Cell::stringFromColumnIndex($col);
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($isActive) {
			$this->_FKExcel->setActiveSheetIndex($this->_FKExcel->getIndex($this->_phpSheet));
		}
		...
		if ($this->_phpSheet->getSheetView()->getView() !== FKExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
			...
			$view = $isPageBreakPreview? FKExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
				FKExcel_Worksheet_SheetView::SHEETVIEW_NORMAL;
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(FKExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(FKExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
			} else {
				// unfrozen panes; split windows; not supported by FKExcel core
			}
		}
	}
	...
	private function _includeCellRangeFiltered($cellRangeAddress)
	{
		...
		if ($this->getReadFilter() !== NULL) {
			...
			$rangeBoundaries = FKExcel_Cell::getRangeBoundaries($cellRangeAddress);
			$rangeBoundaries[1][0]++;
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			foreach (FKExcel_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = FKExcel_Cell_DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = FKExcel_Cell_DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = FKExcel_Cell_DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = FKExcel_Cell_DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = FKExcel_Cell_DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = FKExcel_Cell_DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = FKExcel_Cell_DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = FKExcel_Cell_DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = FKExcel_Cell_DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = FKExcel_Cell_DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = FKExcel_Cell_DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = FKExcel_Cell_DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = FKExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = FKExcel_Cell_DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = FKExcel_Cell_DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = FKExcel_Cell_DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = FKExcel_Cell_DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = FKExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = FKExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}
		...
		try {
			...
			if ($type == FKExcel_Cell_DataValidation::TYPE_LIST) {
		...
		foreach ($cellRangeAddresses as $cellRange) {
			...
			$stRange = FKExcel_Cell::extractAllCellReferencesInRange($stRange);
...
//			var_dump($bcBitCount);
			...
			$drawing = new FKExcel_Worksheet_Drawing();
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x19:	//	Special attribute
			...
			case 0x41:
				...
				case 0x05:
					...
					break;
				default:
					throw new FKExcel_Reader_Exception('Unrecognized space type in tAttrSpace token');
					break;
				}
				...
				break;
			default:
				throw new FKExcel_Reader_Exception('Unrecognized attribute flag in tAttr token');
				break;
			}
			break;
		...
		case 0x61:
			...
			default:
				throw new FKExcel_Reader_Exception('Unrecognized function in formula');
				break;
			}
		...
		default:
			throw new FKExcel_Reader_Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
			break;
		}
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = FKExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));
	...
	{
		list($baseCol, $baseRow) = FKExcel_Cell::coordinateFromString($baseCell);
		$baseCol = FKExcel_Cell::columnIndexFromString($baseCol) - 1;
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = FKExcel_Cell::stringFromColumnIndex($colIndex);
				$column = '$' . $column;
			} else {
				$colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
				$column = FKExcel_Cell::stringFromColumnIndex($baseCol + $colIndex);
			}
	...
	private function _readBIFF5CellRangeAddressFixed($subData)
	{
		...
		// check values
		if ($fr > $lr || $fc > $lc) {
			throw new FKExcel_Reader_Exception('Not a cell range address');
		}

		// column index to letter
		$fc = FKExcel_Cell::stringFromColumnIndex($fc);
		$lc = FKExcel_Cell::stringFromColumnIndex($lc);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// bit: 7-0; mask 0x00FF; column index
		$fc = FKExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 4));
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = FKExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		if (!(0x4000 & self::_GetInt2d($subData, 4))) {
			...
			$fc = FKExcel_Cell::stringFromColumnIndex($fcIndex);
			$fc = '$' . $fc;
		} else {
			// column offset
			$fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
			$fc = FKExcel_Cell::stringFromColumnIndex($baseCol + $fcIndex);
		}
		...
		$lc = FKExcel_Cell::stringFromColumnIndex($baseCol + $lcIndex);

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d($subData, 6))) {
			// absolute column index
			$lc = FKExcel_Cell::stringFromColumnIndex($lcIndex);
			$lc = '$' . $lc;
		} else {
			// column offset
			$lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
	 ...
	 * in which case an FKExcel_Reader_Exception is thrown
	 *
	 * @param int $index
	 * @return string|false
	 * @throws FKExcel_Reader_Exception
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
						throw new FKExcel_Reader_Exception('Deleted sheet reference');
					}
				...
				default:
					...
					throw new FKExcel_Reader_Exception('Excel5 reader only supports internal sheets in fomulas');
					break;
			}
		}
		return false;
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return FKExcel_Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return FKExcel_Shared_String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return FKExcel_Style_Border::BORDER_NONE;
			case 0x01: return FKExcel_Style_Border::BORDER_THIN;
			case 0x02: return FKExcel_Style_Border::BORDER_MEDIUM;
			case 0x03: return FKExcel_Style_Border::BORDER_DASHED;
			case 0x04: return FKExcel_Style_Border::BORDER_DOTTED;
			case 0x05: return FKExcel_Style_Border::BORDER_THICK;
			case 0x06: return FKExcel_Style_Border::BORDER_DOUBLE;
			case 0x07: return FKExcel_Style_Border::BORDER_HAIR;
			case 0x08: return FKExcel_Style_Border::BORDER_MEDIUMDASHED;
			case 0x09: return FKExcel_Style_Border::BORDER_DASHDOT;
			case 0x0A: return FKExcel_Style_Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return FKExcel_Style_Border::BORDER_DASHDOTDOT;
			case 0x0C: return FKExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return FKExcel_Style_Border::BORDER_SLANTDASHDOT;
			default:   return FKExcel_Style_Border::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return FKExcel_Style_Fill::FILL_NONE;
			case 0x01: return FKExcel_Style_Fill::FILL_SOLID;
			case 0x02: return FKExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return FKExcel_Style_Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return FKExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return FKExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return FKExcel_Style_Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return FKExcel_Style_Fill::FILL_PATTERN_DARKUP;
			case 0x09: return FKExcel_Style_Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return FKExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return FKExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return FKExcel_Style_Fill::FILL_PATTERN_GRAY125;
			case 0x12: return FKExcel_Style_Fill::FILL_PATTERN_GRAY0625;
			default:   return FKExcel_Style_Fill::FILL_NONE;
		}
	}
