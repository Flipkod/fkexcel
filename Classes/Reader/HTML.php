 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_HTML extends FKExcel_Reader_Abstract implements FKExcel_Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	private $_formats = array( 'h1' => array( 'font' => array( 'bold' => true,
							   ...
							   'a'  => array( 'font' => array( 'underline' => true,
															   'color' => array( 'argb' => FKExcel_Style_Color::COLOR_BLUE,
															                   ),
															 ),
											),	//	Blue underlined
							   'hr' => array( 'borders' => array( 'bottom' => array( 'style' => FKExcel_Style_Border::BORDER_THIN,
																					 'color' => array( FKExcel_Style_Color::COLOR_BLACK,
																					                 ),
																				   ),
																),
											),	//	Bottom border
	...
	/**
	 * Create a new FKExcel_Reader_HTML
	 */
	...
	//	Data Array used for testing only, should write to FKExcel object on completion of tests
...
//						echo 'END OF TABLE ' , $this->_tableLevel , ' CELL<br />';
	...
	public function loadIntoExisting($pFilename, FKExcel $objFKExcel)
	{
		...
		if (!$this->_isValidFormat()) {
			...
			throw new FKExcel_Reader_Exception($pFilename . " is an Invalid HTML file.");
		}
		...
		if ($loaded === FALSE) {
			throw new FKExcel_Reader_Exception('Failed to load ',$pFilename,' as a DOM Document');
		}
		...
		$column = 'A';
		$content = '';
		$this->_processDomElement($dom,$objFKExcel->getActiveSheet(),$row,$column,$content);
