...
/**
 * FKExcel_Reader_Excel2007
 *
 * @category	FKExcel
 * @package	FKExcel_Reader
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Reader_Excel2007 extends FKExcel_Reader_Abstract implements FKExcel_Reader_IReader
{
	/**
	 * FKExcel_ReferenceHelper instance
	 *
	 * @var FKExcel_ReferenceHelper
	 */
	private $_referenceHelper = NULL;

	/**
	 * FKExcel_Reader_Excel2007_Theme instance
	 *
	 * @var FKExcel_Reader_Excel2007_Theme
	 */
	private static $_theme = NULL;


	/**
	 * Create a new FKExcel_Reader_Excel2007 instance
	 */
	public function __construct() {
		$this->_readFilter = new FKExcel_Reader_DefaultReadFilter();
		$this->_referenceHelper = FKExcel_ReferenceHelper::getInstance();
	}
	...
	public function canRead($pFilename)
	{
		...
		if (!class_exists('ZipArchive',FALSE)) {
			throw new FKExcel_Reader_Exception("ZipArchive library is not enabled");
		}
			...
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				...
				if ($xmlWorkbook->sheets) {
					...
					foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
						...
						$xml = new XMLReader();
						$res = $xml->open('zip://'.FKExcel_Shared_File::realpath($pFilename).'#'."$dir/$fileWorksheet");
...
//				echo 'Formula is '.$sharedFormulas[$instance]['formula'].'<br />';
				$master = FKExcel_Cell::coordinateFromString($sharedFormulas[$instance]['master']);
				$current = FKExcel_Cell::coordinateFromString($r);

				$difference = array(0, 0);
				$difference[0] = FKExcel_Cell::columnIndexFromString($current[0]) - FKExcel_Cell::columnIndexFromString($master[0]);
...
//				echo 'Adjusted Formula is '.$value.'<br />';
	...
	public function _getFromZipArchive(ZipArchive $archive, $fileName = '')
	{
		...
		$fileName = FKExcel_Shared_File::realpath($fileName);
	...
	public function load($pFilename)
	{
		...
		if ($wbRels = simplexml_load_string($this->_getFromZipArchive($zip, "xl/_rels/workbook.xml.rels"))) { //~ http://schemas.openxmlformats.org/package/2006/relationships");
				...
				switch ($rel["Type"]) {
					case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
						...
						if (is_object($xmlTheme)) {
							...
							self::$_theme = new FKExcel_Reader_Excel2007_Theme($themeName,$colourSchemeName,$themeColours);
						}
						break;
				}
			}
					...
					if (is_object($xmlCore)) {
						...
						foreach ($xmlCore as $xmlProperty) {
							...
							if (isset($cellDataOfficeAttributes['name'])) {
								...
								$attributeValue = FKExcel_DocumentProperties::convertProperty($attributeValue,$attributeType);
								$attributeType = FKExcel_DocumentProperties::convertPropertyType($attributeType);
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (isset($xmlStrings) && isset($xmlStrings->si)) {
						foreach ($xmlStrings->si as $val) {
							if (isset($val->t)) {
								$sharedStrings[] = FKExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $val->t );
							} elseif (isset($val->r)) {
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						foreach ($xmlStyles->cellXfs->xf as $xf) {
							$numFmt = FKExcel_Style_NumberFormat::FORMAT_GENERAL;
							...
							if ($xf["numFmtId"]) {
								...
								if ((int)$xf["numFmtId"] < 164) {
									$numFmt = FKExcel_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]);
								}
							}
                            $quotePrefix = false;
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						...
						if ($xmlStyles->dxfs) {
							foreach ($xmlStyles->dxfs->dxf as $dxf) {
								$style = new FKExcel_Style(FALSE, TRUE);
								...
								foreach ($xmlSheet->cols->col as $col) {
									for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
										if ($col["style"] && !$this->_readDataOnly) {
											$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setXfIndex(intval($col["style"]));
										}
										if (self::boolean($col["bestFit"])) {
											//$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setAutoSize(TRUE);
										}
										if (self::boolean($col["hidden"])) {
											$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setVisible(FALSE);
										}
										if (self::boolean($col["collapsed"])) {
											$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setCollapsed(TRUE);
										}
										if ($col["outlineLevel"] > 0) {
											$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setOutlineLevel(intval($col["outlineLevel"]));
										}
										$docSheet->getColumnDimension(FKExcel_Cell::stringFromColumnIndex($i))->setWidth(floatval($col["width"]));
								...
								foreach ($xmlSheet->sheetData->row as $row) {
									...
									foreach ($row->c as $c) {
										...
										if ($this->getReadFilter() !== NULL) {
											$coordinates = FKExcel_Cell::coordinateFromString($r);
	...
	//											echo 'String<br />';
												if ((string)$c->v != '') {
													...
													if ($value instanceof FKExcel_RichText) {
														$value = clone $value;
													}
												} else {
													$value = '';
	...
	//									echo 'Value is '.$value.'<br />';
										...
										// Rich text?
										if ($value instanceof FKExcel_RichText && $this->_readDataOnly) {
							...
							if (!$this->_readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
								foreach ($xmlSheet->conditionalFormatting as $conditional) {
									foreach ($conditional->cfRule as $cfRule) {
										if (
											(
												(string)$cfRule["type"] == FKExcel_Style_Conditional::CONDITION_NONE ||
												(string)$cfRule["type"] == FKExcel_Style_Conditional::CONDITION_CELLIS ||
												(string)$cfRule["type"] == FKExcel_Style_Conditional::CONDITION_CONTAINSTEXT ||
												(string)$cfRule["type"] == FKExcel_Style_Conditional::CONDITION_EXPRESSION
								...
								foreach ($conditionals as $ref => $cfRules) {
									...
									foreach ($cfRules as $cfRule) {
										$objConditional = new FKExcel_Style_Conditional();
									...
									$aReferences = FKExcel_Cell::extractAllCellReferencesInRange($ref);
							...
							if ($xmlSheet && $xmlSheet->autoFilter && !$this->_readDataOnly) {
								...
								foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
									...
									if ($filterColumn->filters) {
										$column->setFilterType(FKExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER);
										...
										if ((isset($filters["blank"])) && ($filters["blank"] == 1)) {
											...
											->setRuleType(FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_FILTER);
										}
										...
										foreach ($filters->dateGroupItem as $dateGroupItem) {
											...
											->setRuleType(FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DATEGROUP);
										}
									}
									//	Check for custom filters
									if ($filterColumn->customFilters) {
										$column->setFilterType(FKExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER);
										$customFilters = $filterColumn->customFilters;
										//	Custom filters can an AND or an OR join;
										//		and there should only ever be one or two entries
										if ((isset($customFilters["and"])) && ($customFilters["and"] == 1)) {
											$column->setJoin(FKExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND);
										}
										foreach ($customFilters->customFilter as $filterRule) {
											...
											->setRuleType(FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);
										}
									}
									//	Check for dynamic filters
									if ($filterColumn->dynamicFilter) {
										$column->setFilterType(FKExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER);
										...
										foreach ($filterColumn->dynamicFilter as $filterRule) {
											...
											->setRuleType(FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
									...
									if ($filterColumn->top10) {
										$column->setFilterType(FKExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER);
										//	We should only ever have one top10 filter
										foreach ($filterColumn->top10 as $filterRule) {
											$column->createRule()->setRule(
												(((isset($filterRule["percent"])) && ($filterRule["percent"] == 1))
													? FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT
													: FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE
												),
												(string) $filterRule["val"],
												(((isset($filterRule["top"])) && ($filterRule["top"] == 1))
													? FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP
													: FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM
												)
											)
											->setRuleType(FKExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_TOPTENFILTER);
										}
									}
								}
							}
							...
							if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->rowBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak("A$brk[id]", FKExcel_Worksheet::BREAK_ROW);
									}
								}
							}
							if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->colBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak(FKExcel_Cell::stringFromColumnIndex((string) $brk["id"]) . "1", FKExcel_Worksheet::BREAK_COLUMN);
									}
								}
							}
							...
							if ($xmlSheet && $xmlSheet->dataValidations && !$this->_readDataOnly) {
								foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
									...
									foreach($rangeSet as $range) {
										...
										$aReferences = FKExcel_Cell::extractAllCellReferencesInRange($stRange);
										foreach ($aReferences as $reference) {
											// Create validation
							...
							if (!$this->_readDataOnly) {
								...
								if ($xmlSheet && $xmlSheet->hyperlinks) {
									foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
										...
										foreach (FKExcel_Cell::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									// Load comments file
									$relPath = FKExcel_Shared_File::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
									...
									if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
										...
										if ($vmlRelationship != '') {
											...
											foreach ($shapes as $shape) {
												...
												$hfImages[ (string)$shape['id'] ] = new FKExcel_Worksheet_HeaderFooterDrawing();
												if (isset($imageData['title'])) {
													$hfImages[ (string)$shape['id'] ]->setName( (string)$imageData['title'] );
												}

												$hfImages[ (string)$shape['id'] ]->setPath("zip://".FKExcel_Shared_File::realpath($pFilename)."#" . $drawings[(string)$imageData['relid']], false);
										...
										if ($xmlDrawing->oneCellAnchor) {
											foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													$objDrawing = new FKExcel_Worksheet_Drawing;
													...
													$objDrawing->setPath("zip://".FKExcel_Shared_File::realpath($pFilename)."#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
													$objDrawing->setCoordinates(FKExcel_Cell::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1));
													$objDrawing->setOffsetX(FKExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->colOff));
													$objDrawing->setOffsetY(FKExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);
													$objDrawing->setWidth(FKExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx")));
													$objDrawing->setHeight(FKExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy")));
													if ($xfrm) {
														$objDrawing->setRotation(FKExcel_Shared_Drawing::angleToDegrees(self::array_item($xfrm->attributes(), "rot")));
													}
													if ($outerShdw) {
														$shadow = $objDrawing->getShadow();
														$shadow->setVisible(true);
														$shadow->setBlurRadius(FKExcel_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "blurRad")));
														$shadow->setDistance(FKExcel_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "dist")));
														$shadow->setDirection(FKExcel_Shared_Drawing::angleToDegrees(self::array_item($outerShdw->attributes(), "dir")));
													...
													$coordinates	= FKExcel_Cell::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1);
													$offsetX		= FKExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->colOff);
													$offsetY		= FKExcel_Shared_Drawing::EMUToPixels($oneCellAnchor->from->rowOff);
													$width			= FKExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx"));
													$height			= FKExcel_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy"));
												}
											}
										}
										if ($xmlDrawing->twoCellAnchor) {
											foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
												if ($twoCellAnchor->pic->blipFill) {
													...
													$objDrawing->setPath("zip://".FKExcel_Shared_File::realpath($pFilename)."#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
													$objDrawing->setCoordinates(FKExcel_Cell::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1));
													$objDrawing->setOffsetX(FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->colOff));
													$objDrawing->setOffsetY(FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);

													$objDrawing->setWidth(FKExcel_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cx")));
													$objDrawing->setHeight(FKExcel_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cy")));
													...
													$fromCoordinate	= FKExcel_Cell::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1);
													$fromOffsetX	= FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->colOff);
													$fromOffsetY	= FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->from->rowOff);
													$toCoordinate	= FKExcel_Cell::stringFromColumnIndex((string) $twoCellAnchor->to->col) . ($twoCellAnchor->to->row + 1);
													$toOffsetX		= FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->to->colOff);
													$toOffsetY		= FKExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->to->rowOff);
							...
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
											...
											if ($mapSheetId[(integer) $definedName['localSheetId']] !== null) {
												...
												if (count($range) == 2) {
													...
													if ($worksheet = $docSheet->getParent()->getSheetByName($range[0])) {
														...
														$excel->addNamedRange( new FKExcel_NamedRange((string)$definedName['name'], $worksheet, $extractedRange, true, $scope) );
													}
												}
											}
											break;
									...
									if (strpos( (string)$definedName, '!' ) !== false) {
										// Extract sheet name
										$extractedSheetName = FKExcel_Worksheet::extractSheetTitle( (string)$definedName, true );
									...
									if ($locatedSheet !== NULL) {
										$excel->addNamedRange( new FKExcel_NamedRange((string)$definedName['name'], $locatedSheet, $extractedRange, false) );
									}
								}
							}
						}
				...
				switch ($contentType["ContentType"]) {
					case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
						if ($this->_includeCharts) {
							...
							$objChart = FKExcel_Reader_Excel2007_Chart::readChart($chartElements,basename($chartEntryRef,'.xml'));
...
//									var_dump($chartDetails[$chartPositionRef]);
	...
	private static function _readColor($color, $background=FALSE) {
		if (isset($color["rgb"])) {
			...
			return FKExcel_Style_Color::indexedColor($color["indexed"]-7,$background)->getARGB();
		} else if (isset($color["theme"])) {
			if (self::$_theme !== NULL) {
				$returnColour = self::$_theme->getColourByIndex((int)$color["theme"]);
				if (isset($color["tint"])) {
					$tintAdjust = (float) $color["tint"];
					$returnColour = FKExcel_Style_Color::changeBrightness($returnColour, $tintAdjust);
				}
...
//		}
		...
		if (isset($style->font)) {
			...
			if (isset($style->font->u) && !isset($style->font->u["val"])) {
				$docStyle->getFont()->setUnderline(FKExcel_Style_Font::UNDERLINE_SINGLE);
		...
		if (isset($style->border)) {
			...
			if (!$diagonalUp && !$diagonalDown) {
				$docStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_NONE);
			} elseif ($diagonalUp && !$diagonalDown) {
				$docStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_UP);
			} elseif (!$diagonalUp && $diagonalDown) {
				$docStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_DOWN);
			} else {
				$docStyle->getBorders()->setDiagonalDirection(FKExcel_Style_Borders::DIAGONAL_BOTH);
			}
		...
		if (isset($style->protection)) {
			if (isset($style->protection['locked'])) {
				if (self::boolean((string) $style->protection['locked'])) {
					$docStyle->getProtection()->setLocked(FKExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$docStyle->getProtection()->setLocked(FKExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}

			if (isset($style->protection['hidden'])) {
				if (self::boolean((string) $style->protection['hidden'])) {
					$docStyle->getProtection()->setHidden(FKExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$docStyle->getProtection()->setHidden(FKExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}
		}
	...
	private function _parseRichText($is = null) {
		...
		if (isset($is->t)) {
			$value->createText( FKExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $is->t ) );
		} else {
			foreach ($is->r as $run) {
				if (!isset($run->rPr)) {
					$objText = $value->createText( FKExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );

				} else {
					$objText = $value->createTextRun( FKExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );
					...
					if (isset($run->rPr->color)) {
						$objText->getFont()->setColor( new FKExcel_Style_Color( self::_readColor($run->rPr->color) ) );
					}
					...
					if (isset($run->rPr->u) && !isset($run->rPr->u["val"])) {
						$objText->getFont()->setUnderline(FKExcel_Style_Font::UNDERLINE_SINGLE);
	...
	private static function toCSSArray($style) {
		...
		foreach ($temp as $item) {
			...
			if (strpos($item[1], 'pt') !== false) {
				...
				$item[1] = FKExcel_Shared_Font::fontSizeToPixels($item[1]);
			}
			if (strpos($item[1], 'in') !== false) {
				$item[1] = str_replace('in', '', $item[1]);
				$item[1] = FKExcel_Shared_Font::inchSizeToPixels($item[1]);
			}
			if (strpos($item[1], 'cm') !== false) {
				$item[1] = str_replace('cm', '', $item[1]);
				$item[1] = FKExcel_Shared_Font::centimeterSizeToPixels($item[1]);
			}
