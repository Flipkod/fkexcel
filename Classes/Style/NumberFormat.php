...
/**
 * FKExcel_Style_NumberFormat
 *
 * @category   FKExcel
 * @package	FKExcel_Style
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Style_NumberFormat extends FKExcel_Style_Supervisor implements FKExcel_IComparable
{
	/* Pre-defined formats */
	 ...
	 * @var string
	 */
	protected $_formatCode	=	FKExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new FKExcel_Style_NumberFormat
	 *
	 ...
	 *
	 * <code>
	 * $objFKExcel->getActiveSheet()->getStyle('B2')->getNumberFormat()->applyFromArray(
	 *		array(
	 *			'code' => FKExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return FKExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = FKExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = FKExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = FKExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$dateObj = FKExcel_Shared_Date::ExcelToPHPObject($value);
	...
	private static function _formatAsFraction(&$value, &$format)
	{
		...
		$GCD = FKExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
	 ...
	 */
	public static function toFormattedString($value = '0', $format = FKExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		if (($format === FKExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === FKExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return $value;
		}
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
										$value
										, strlen($right)
										, FKExcel_Shared_String::getDecimalSeparator()
										, FKExcel_Shared_String::getThousandsSeparator()
									);
