...
/**
 * FKExcel_Chart_DataSeriesValues
 *
 * @category	FKExcel
 * @package		FKExcel_Chart
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Chart_DataSeriesValues
{
	...
	/**
	 * Create a new FKExcel_Chart_DataSeriesValues object
	 */
	 ...
	 *								Typical values are:
	 *									FKExcel_Chart_DataSeriesValues::DATASERIES_TYPE_STRING
	 *										Normally used for axis point values
	 *									FKExcel_Chart_DataSeriesValues::DATASERIES_TYPE_NUMBER
	 *										Normally used for chart data values
	 * @return	FKExcel_Chart_DataSeriesValues
	 */
	public function setDataType($dataType = self::DATASERIES_TYPE_NUMBER) {
		if (!in_array($dataType, self::$_dataTypeValues)) {
    		throw new FKExcel_Chart_Exception('Invalid datatype for chart data series values');
		}
	...
	public function setDataValues($dataValues = array(), $refreshDataSource = TRUE) {
		$this->_dataValues = FKExcel_Calculation_Functions::flattenArray($dataValues);
	...
	public function refresh(FKExcel_Worksheet $worksheet, $flatten = TRUE) {
        if ($this->_dataSource !== NULL) {
        	$calcEngine = FKExcel_Calculation::getInstance($worksheet->getParent());
			$newDataValues = FKExcel_Calculation::_unwrapResult(
			...
			if ($flatten) {
				$this->_dataValues = FKExcel_Calculation_Functions::flattenArray($newDataValues);
				...
				$dimensions = FKExcel_Cell::rangeDimension(str_replace('$','',$cellRange));
