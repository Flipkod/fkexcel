 ...
 *
 * @category	FKExcel
 * @package		FKExcel_Chart_Renderer
...
require_once(FKExcel_Settings::getChartRendererPath().'/jpgraph.php');


/**
 * FKExcel_Chart_Renderer_jpgraph
 *
 * @category	FKExcel
 * @package		FKExcel_Chart_Renderer
 * @copyright	Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Chart_Renderer_jpgraph
{
	...
	private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '') {
		...
		foreach($datasetLabels as $i => $datasetLabel) {
			if (is_array($datasetLabel)) {
				...
				if (!is_null($datasetLabelFormatCode)) {
					$datasetLabels[$i] = FKExcel_Style_NumberFormat::toFormattedString($datasetLabel,$datasetLabelFormatCode);
				}
			}
			++$testCurrentIndex;
		}
	...
	/**
	 * Create a new FKExcel_Chart_Renderer_jpgraph
	 */
	public function __construct(FKExcel_Chart $chart)
	{
		$this->_graph	= null;
		$this->_chart	= $chart;
