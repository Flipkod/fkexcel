 ...
 * @copyright  Copyright (c) 2006 - 2013 FKExcel (http://www.codeplex.com/FKExcel)
 */
class FKExcel_Style extends FKExcel_Style_Supervisor implements FKExcel_IComparable
{
    /**
     * Font
     *
     ...
     * Number Format
     *
     * @var FKExcel_Style_NumberFormat
     */
    ...
    public function __construct($isSupervisor = false, $isConditional = false)
    {
        ...
        $this->_font              = new FKExcel_Style_Font($isSupervisor, $isConditional);
        $this->_fill              = new FKExcel_Style_Fill($isSupervisor, $isConditional);
        $this->_borders           = new FKExcel_Style_Borders($isSupervisor, $isConditional);
        $this->_alignment         = new FKExcel_Style_Alignment($isSupervisor, $isConditional);
        $this->_numberFormat      = new FKExcel_Style_NumberFormat($isSupervisor, $isConditional);
        $this->_protection        = new FKExcel_Style_Protection($isSupervisor, $isConditional);
     ...
     *
     * <code>
     * $objFKExcel->getActiveSheet()->getStyle('B2')->applyFromArray(
     *         array(
     *             'font'    => array(
     *                 'name'      => 'Arial',
     *                 'bold'      => true,
     *                 'italic'    => false,
     *                 'underline' => FKExcel_Style_Font::UNDERLINE_DOUBLE,
     *                 'strike'    => false,
     *                 'color'     => array(
     *                     'rgb' => '808080'
     *                 )
     ...
     *             'borders' => array(
     *                 'bottom'     => array(
     *                     'style' => FKExcel_Style_Border::BORDER_DASHDOT,
     *                     'color' => array(
     *                         'rgb' => '808080'
     *                     )
     *                 ),
    ...
    public function applyFromArray($pStyles = null, $pAdvanced = true)
    {
        if (is_array($pStyles)) {
            if ($this->_isSupervisor) {
                ...
                $rangeStart = FKExcel_Cell::coordinateFromString($rangeA);
                $rangeEnd     = FKExcel_Cell::coordinateFromString($rangeB);

                // Translate column into index
                $rangeStart[0]    = FKExcel_Cell::columnIndexFromString($rangeStart[0]) - 1;
                $rangeEnd[0]    = FKExcel_Cell::columnIndexFromString($rangeEnd[0]) - 1;
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        $colStart = ($x == 3) ?
                            FKExcel_Cell::stringFromColumnIndex($rangeEnd[0])
                                : FKExcel_Cell::stringFromColumnIndex($rangeStart[0] + $x - 1);

                        // end column index for region
                        $colEnd = ($x == 1) ?
                            FKExcel_Cell::stringFromColumnIndex($rangeStart[0])
                                : FKExcel_Cell::stringFromColumnIndex($rangeEnd[0] - $xMax + $x);
     ...
     * @return FKExcel_Style
     */
    public function setFont(FKExcel_Style_Font $font)
    {
        $this->_font = $font;
        return $this;
    }
     ...
     * Get Alignment
     *
     * @return FKExcel_Style_Alignment
     */
     ...
     * Get Number Format
     *
     * @return FKExcel_Style_NumberFormat
     */
     ...
     *
     * @param FKExcel_Style_Conditional[] $pValue Array of condtional styles
     * @return FKExcel_Style
     */
     ...
     * Get Protection
     *
     * @return FKExcel_Style_Protection
     */
