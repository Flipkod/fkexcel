...
class FKExcel_Shared_OLE
{
	...
	*/
	public function read($file)
	{
		$fh = fopen($file, "r");
		if (!$fh) {
			throw new FKExcel_Reader_Exception("Can't open file $file");
		}
		$this->_file_handle = $fh;

		$signature = fread($fh, 8);
		if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) {
			throw new FKExcel_Reader_Exception("File doesn't seem to be an OLE container.");
		}
		fseek($fh, 28);
		if (fread($fh, 2) != "\xFE\xFF") {
			// This shouldn't be a problem in practice
			throw new FKExcel_Reader_Exception("Only Little-Endian encoding is supported.");
		}
	...
	* use FKExcel_Shared_OLE_PPS_File::getStream().
	...
	public function getStream($blockIdOrPps)
	{
		...
		if (!$isRegistered) {
			stream_wrapper_register('ole-chainedblockstream',
				'FKExcel_Shared_OLE_ChainedBlockStream');
			$isRegistered = true;
		}
		...
		if ($blockIdOrPps instanceof FKExcel_Shared_OLE_PPS) {
	...
	public function _readPpsWks($blockId)
	{
		...
		for ($pos = 0; ; $pos += 128) {
			...
			case self::OLE_PPS_TYPE_ROOT:
				$pps = new FKExcel_Shared_OLE_PPS_Root(null, null, array());
				$this->root = $pps;
				break;
			case self::OLE_PPS_TYPE_DIR:
				$pps = new FKExcel_Shared_OLE_PPS(null, null, null, null, null,
								   null, null, null, null, array());
				break;
			case self::OLE_PPS_TYPE_FILE:
				$pps = new FKExcel_Shared_OLE_PPS_File($name);
				break;
			default:
				continue;
			}
