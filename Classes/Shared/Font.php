...
class FKExcel_Shared_Font
{
	 ...
	 *
	 * @param 	FKExcel_Style_Font			$font			Font object
	 * @param 	FKExcel_RichText|string	$cellText		Text to calculate width
	 * @param 	integer						$rotation		Rotation angle
	 * @param 	FKExcel_Style_Font|NULL	$defaultFont	Font object
	 * @return 	integer		Column width
	 */
	public static function calculateColumnWidth(FKExcel_Style_Font $font, $cellText = '', $rotation = 0, FKExcel_Style_Font $defaultFont = null) {

		// If it is rich text, use plain text
		if ($cellText instanceof FKExcel_RichText) {
		...
		try {
			...
			if (self::$autoSizeMethod == self::AUTOSIZE_METHOD_APPROX) {
				throw new FKExcel_Exception('AutoSize method is set to approx');
			}
		...
		$columnWidth = FKExcel_Shared_Drawing::pixelsToCellDimension($columnWidth, $defaultFont);
	 ...
	 *
	 * @param string $text
	 * @param FKExcel_Style_Font
	 * @param int $rotation
	 * @return int
	 * @throws FKExcel_Exception
	 */
	public static function getTextWidthPixelsExact($text, FKExcel_Style_Font $font, $rotation = 0) {
		if (!function_exists('imagettfbbox')) {
			throw new FKExcel_Exception('GD library needs to be enabled');
		}
	 ...
	 */
	public static function getTextWidthPixelsApprox($columnText, FKExcel_Style_Font $font = null, $rotation = 0)
	{
		...
		switch ($fontName) {
			case 'Calibri':
				...
				$columnWidth = (int) (8.26 * FKExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 11; // extrapolate from font size
				break;

			case 'Arial':
				// value 7 was found via interpolation by inspecting real Excel files with Arial 10 font.
				$columnWidth = (int) (7 * FKExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;

			case 'Verdana':
				// value 8 was found via interpolation by inspecting real Excel files with Verdana 10 font.
				$columnWidth = (int) (8 * FKExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;
	...
	public static function getTrueTypeFontFileFromFont($font) {
		if (!file_exists(self::$trueTypeFontPath) || !is_dir(self::$trueTypeFontPath)) {
			throw new FKExcel_Exception('Valid directory to TrueType Font files not specified');
		}
		...
		switch ($name) {
			...
			default:
				throw new FKExcel_Exception('Unknown font name "'. $name .'". Cannot map to TrueType font file');
				break;
		}
		...
		if (!file_exists($fontFile)) {
			throw New FKExcel_Exception('TrueType Font file not found');
		}
	 ...
	 *
	 * @param FKExcel_Style_Font $font The workbooks default font
	 * @param boolean $pPixels true = return column width in pixels, false = return in OOXML units
	 * @return mixed Column width
	 */
	public static function getDefaultColumnWidthByFont(FKExcel_Style_Font $font, $pPixels = false)
	{
	 ...
	 */
	public static function getDefaultRowHeightByFont(FKExcel_Style_Font $font)
	{
		switch ($font->getName()) {
			case 'Arial':
				switch ($font->getSize()) {
